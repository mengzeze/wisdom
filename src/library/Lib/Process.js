...
class Process{
    constructor(argument={}){
        ...
        this.operate = argument.operate ? argument.operate : 0 // 0为非查询按钮 1为查询按钮
        ...
        let arr = filter ? this.storage.filter(item=>{
            ...
            for(let i in filter){
                if (filter[i] && filter[i] !== '') {
                    console.log(filter[i], '_____________progStageId_____filter[i]')
                    if (i === 'available' && filter[i] !== 'R222' && item[i].toString() !== filter[i].toString()) {
                      ...
                      num++
                    } else if ((item[i] === null || typeof item[i] === 'undefined') && filter[i] !== null && filter[i] !== 'R222') {
                      num++
                    } else if (typeof item[i] === 'undefined' && filter[i] !== null && filter[i] !== 'R222') {
                      num++
                    }
                  }
...
            "pageNo": this.pageNo,
    ...
    query(object={},dir){
        ...
        let singleProcType = this.operate === 1 ? true : ''
