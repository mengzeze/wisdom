        ...
        <span>
          <div class="auditorType clearfix">
            ...
            <div class="typeRight fr">
              ...
              <div style="height:380px;overflow: auto;border:1px solid #ddd;padding:10px;">
                <ul class="choTypes" v-if="olarr != ''">
                  <li class="clearfix" v-for="(item,index) in olarr">
                    <span class="ifnnos"   :title="item.name">{{item.name}}</span>
                    <i @click="delect(item,index)" class="el-icon-delete fr"></i>
                  </li>
                </ul>
                <p class="pop_no_select_item_tips" v-else>暂无选中项</p>
              </div>
            </div>
          </div>
        </span>
...
export default {
  ...
  props: ["flag","flagstate","selectTree","typeTit","rights","setdatainfo"],
  watch: {
  ...
  methods: {
    ...
    handleNodeClick(data) {
      ...
      let obj = {};
      this.olarr = this.olarr.reduce((cur, next) => {
