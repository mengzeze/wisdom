<template>
  <!--多选-->
  <div class="findall-parent" v-if="findFlagShow">
      <el-dialog title="选择人员" :close-on-click-modal="false" :visible="findFlagShow" width="963px" @close="cancelBtn" class="find_box">
...
<script>
  export default {
    ...
    props: ['findFlagShow', 'findState', 'checkState', 'findUserObj', 'roleSelect', 'isSelectMember'],
    components: {
    },
    computed: {
      searchSite() {
    ...
    methods: {
      ...
      handleNodeClick (data) {
        console.log(data, this.optionUser)
        if (!data.children && !data.lists) {
          console.log(111, data.uniqueKey)
      ...
      roleQuery() {
        ...
        this.$post('/sys/getQueryRole', obj).then(res => {
          ...
          this.roleDataCopy =  this.roleData
        }).catch(error => {
            console.log(error);
          });
      },
      ...
      roleNodeQuery(node, type) {
        console.log(node, 'roleNodeQuery', type)
        node.isLoading = true
        let obj = {
          token:this.token,
          userId: this.userId,
        ...
        this.$post('/sys/listUserByRoleId', obj).then(res => {
          ...
          //   return
          // }
          if (type && node.children.length > 0) {
      ...
      displayChecked(item){
        ...
        if(item==='all'){
          ...
          data.forEach(v=>{
            v.children.length && this.displayChecked(v)
          })
          return
        }
        ...
        console.log(checked, checkedLen, childrenLen)
    ...
    watch:{
      findFlagShow (val) {
        if (val) {
          ...
          // this.projectId = (this.voteprojId !== "") ? this.voteprojId : this.$store.state.projectMsg.pro_id
          console.log(this.findUserObj, '接收到的人员值',  this.roleSelect)
          this.findUserObj && this.findUserObj.length ? this.optionUser = JSON.parse(JSON.stringify(this.findUserObj))  : this.optionUser = []
