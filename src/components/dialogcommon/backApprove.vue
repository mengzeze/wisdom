                            ...
                            <div style="border:1px solid #ddd;padding:12px;height:380px;">
                                <el-scrollbar style="height:100%">
                                    <p class="clearfix mr-5" v-for="(item,index) in optArr" v-if="null_flag == false">
                                      <span class="fl" style="width: calc(100% - 20px);text-overflow: ellipsis;overflow: hidden;white-space: nowrap;text-align:left;" :title="item.label">{{item.label}}</span>
                                      <i class="el-icon-delete fr" @click="deleteOpt(index)" style="cursor: pointer"></i>
                                    </p>
...
<script>
    export default {
        ...
        props:['typeObjolg',"stateolg","optStateolg","isProcessengineClick","singleSele","hasSelect"],//singleSele:1/2单选/多选
        created(){
            this.opStateolg = this.optStateolg
          ...
          handleOpen() {
            ...
            sureType(){
                this.optArr.forEach(v=>{
                    v.fields.data.id <= 1 ? v.isOrdinary = true : v.isOrdinary = false
                })
                this.$emit('typeProjectolg',this.optArr);
