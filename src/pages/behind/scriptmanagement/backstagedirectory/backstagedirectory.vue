  ...
  <div class="backstagedirectory">
    <div class="backstagedirectory_contenti_headers">
      <el-breadcrumb separator="/" style="height:46px;line-height:46px">
        ...
        <el-breadcrumb-item :to="{ path: '/backstagedirectorylist' }"
          >目录管理</el-breadcrumb-item
        >
        <el-breadcrumb-item :title="parent_docName">{{
          parent_docName
        }}</el-breadcrumb-item>
      </el-breadcrumb>
    ...
    <div class="contenti_container">
      ...
      <div class="right">
        <div id="right_box">
          <div class="right_header" v-show="!isedit">
            <div class="right_header_l">
              <p
                class="right_header_title ellipsis1 bold"
                :title="parent_docName"
              >
                {{ parent_docName }}
              </p>
              <p class="right_header_title ellipsis1" :title="financingName">
                业务类型：{{ financingName }}
              </p>
            ...
            <div class="right_header_r">
              <el-button
                v-if="$utils.checkSystemPermission('bask_edit_atalog')"
                type="primary"
                @click="iseditFn"
              >
          ...
          <div class="right_header_edit" v-show="isedit">
            <el-row>
              ...
              <el-col :span="7">
                <el-input
                  v-model="save_parent_data.docName"
                  maxlength="1000"
                ></el-input>
              </el-col>
              <el-col :offset="9" :span="6" style="text-align:right">
                <el-button type="primary" @click="saveParentDataFn"
                  >保存</el-button
                >
            ...
            <el-row style="margin-top:10px;">
              ...
              <el-col :span="7">
                <div class="choose-input">
                  ...
                  ></el-input>
                  <div>
                    <el-button @click="businessTypeClick" type="primary"
                      >选择</el-button
                    >
                  </div>
                </div>
            ...
            <el-row class="right_header_edit_bottom">
              ...
              <el-col :span="22">
                <el-input
                  ...
                  type="textarea"
                  placeholder="请输入备注"
                  v-model="save_parent_data.remarks"
                  resize="none"
                  maxlength="400"
                ></el-input>
              </el-col>
        ...
        <div class="right_header_second">
          <p class="right_header_second_title">
            ...
            <span :title="leftTreeSelectData.docName">{{
              leftTreeSelectData.docName
            }}</span>
          </p>
          ...
          <el-scrollbar style="height:100%">
            ...
            >
              ...
              <el-col :span="3" class="operating_box">
                <div class="opera_btn_box">
                  <div
                    class="icon-operate-btn iconfont bianji2-copy"
                    title="编辑"
                    @click="editSonName(item, index)"
                  >
                    <!-- <i class="el-icon-edit-outline"></i> -->
                  </div>
                  <div
                    class="icon-operate-btn iconfont shangjiantou"
                    title="上移"
                    @click="upMove(item, index)"
                  >
                    <!-- <i class="el-icon-back"></i> -->
                  </div>
                  <div
                    class="icon-operate-btn iconfont xiajiantou"
                    title="下移"
            ...
            <el-row :gutter="0" class="tb_row add_row">
              <el-col :span="3" class="check_col">
                <el-checkbox
                  :label="right_direct_tbdata.length + 1"
                ></el-checkbox>
              </el-col>
              <el-col :span="16" class="name_col">
                <input
                  ...
                  placeholder="请输入目录名称"
                  maxlength="1000"
                  @blur="addDocNameBlurFn"
                  @keyup.enter="addDocNameBlurFn"
                />
              </el-col>
            </el-row>
          </el-scrollbar>
      ...
      >
        <div style="height:80px;">
          <el-scrollbar style="height:100%"
            >删除后，将删除该目录下的所有子目录,是否确认删除？</el-scrollbar
          >
        </div>
        <span slot="footer" class="dialog-footer">
      ...
      <el-dialog
        ...
        title="注意事项"
        :close-on-click-modal="false"
        :visible.sync="acction_dialogVisible"
        width="500px"
      >
  ...
  methods: {
    pageHeight() {
      this.$nextTick(() => {
        document.getElementById("left").style.height =
          this.getViewPortHeight() - 60 - 116 + "px";
        document.querySelector(".right").style.height =
          this.getViewPortHeight() - 60 - 116 + "px";
        document.getElementById("right_content").style.height =
          document.querySelector(".right").clientHeight -
          document.getElementById("right_box").clientHeight -
          91 +
          "px";
    ...
    temp_download(data) {
      ...
      this.$post("/doc/paper/docTemplateDownLogRecord", send)
        .then(response => {
          window.open(this.$utils.getDownloadUrl(data.docId));
        })
        .catch(function(error) {
          console.log(error);
        });
    ...
    saveacction() {
      ...
      if (!this.acction_select_data.id) {
        for (var i = 0; i < this.add_template_file_list.length; i++) {
          if (
            this.acction_select_data.docId ==
            this.add_template_file_list[i].docId
          ) {
    ...
    saveParentDataFn() {
      ...
      if (
        this.businessData.id != "" &&
        this.businessData.id != this.financingId
      ) {
    ...
    editSonName(itemValue, index) {
      ...
      $(".name_col")
        ...
        .css("display", "none");
      var dom = $(
        "<input class='edit_name_input' maxlength=1000 type='text' value='" +
          str +
          "' />"
      );
      $(".name_col")
        ...
        .append(dom);
      // 解决光标在最前问题
      let inputVal = $(".edit_name_input").val();
      $(".edit_name_input")
        .val("")
        .focus()
        .val(inputVal);
...
.backstagedirectory {
  ...
  .contenti_container .close {
    width: 30px;
    height: 15px;
    background: url("../../../../assets/project_doc/treenoopen.png") no-repeat 0
      0;
    display: inline-block;
    &::before {
      content: "";
