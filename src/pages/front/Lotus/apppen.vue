<template>
  <div class="content approval">
    <el-dialog :title="catItem.procName+'审批'" center top="80px" :close-on-click-modal="false" @close="closes" :visible.sync="dialogVisible" width="700px">
      <div class="project" style="height: calc(100vh - 300px);">
        <el-scrollbar style="height:100%">
          <!-- <p></p> -->
          <div class="infowan">
            <p style="float: left;">发起人：{{chemsg}}</p>
            <p class="infowan-type" style="margin-right: 0">业务类型:{{currentgeneration}}</p>
            <p class="infowan-type" style="margin-top: 10px;margin-right: 0">当前阶段:{{Typesoffinancing}}</p>
            <p class="infowan-type" style="margin-top: 10px;float: left;">审批类型:{{Typeapproval}}</p>
          </div>
          <div class="project1">
            ...
            <div v-if="newmesageproject">
              ...
              <ul class="projectnewmessage clearFix" v-if="this.$utils.m('customer_manage')">
                ...
                <li class="clearFix">
                  <div class="messcontent clearFix" v-for="(item,key,index) in messageObj" :key="index">
                    <label for="">{{key}}：</label>
                    <div v-for="(obj, ind) in item" :key="ind" style="float:left;">
                      <span>{{obj.name}}、</span>
                    </div>
                  </div>
                </li>
              ...
              <ul class="projectnewmessage clearFix">
                ...
                <li class="clearFix">
                  <div class="messcontent" v-for="(value,key) in projectMemberInfo" :key="key">
                    <label for="">{{value.roleName}}：</label>
                    <div v-for="(item, index) in value.usNameList" :key="index" class="messDiv">
                      <span>{{item}}、</span>
                    </div>
                  </div>
                </li>
          ...
          <!-- <div v-if="selewenjian != '新建项目审批'">
            <p style="font-weight:400;text-align:left;float: left;margin-left: 15px;">
              申请内容：
            </p>
            <div style="float: left;margin-left: 85px;width: 556px;margin-top: -25px;margin-bottom: 10px;">
              <approvalpending :html2="html2" :changes="changes" />
            </div>
          </div> -->
          <!-- 2-3 -->
          <div class="wait_shfile" style="margin-left:-9px;" v-if="states">
            ...
            <ul id="project2" class="wait_shfile" style="margin-left:15px;height:115px;width: 97%;">
              <el-scrollbar style="height:100%">
                <li v-for="(item,index) in typeText.attachment" v-if="item.delFlag == 0" :key="index" style="height:20px;">
          ...
          <div class="project1" id="auditorfujian" v-if="audfujian">
            <!-- <div class="title_lable">审批附件</div> -->
             ...
             <div class="approval-check">
                    ...
                    <el-checkbox-group class="approval-group" v-model="annex" @change="handleCheckedCitiesChange">
                        <ul>
                            ...
                            <el-checkbox v-for="item in typeText.annex" class="attachemnt-list-item select-box approval-upload" :key="item.fileId" :label="item.fileId">
                                <span class="doc-name" @click="yulan5(item)" :title="item.fileName">{{item.fileName}}</span>
                            </el-checkbox>
                        </ul>
                    </el-checkbox-group>
                </div>
          ...
          <div class="project3 sphj_box">
            <div class="project1_titlebox">
              ...
              <p @click="approved(itemState[1])" class="project2span color-primary" style="cursor: pointer;">&nbsp;审批记录</p>
            </div>
            <!-- <div>审批环节</div> -->
            <ul>
          ...
          <div class="proend" style="margin-top:18px">
           ...
           <div class="proend" style="margin-top:18px;max-height: 126px;overflow-y: auto;height:auto;">
               <p style="padding-bottom: 7px;margin-top:10px;">抄送人:</p>
                ...
                <el-button type="primary" @click="optPrinFnputong" size="small" style="margin-top:-1px;">选择人员</el-button>
                    <el-tag v-for="(item,index) in colyObjputong" :key="'info2'+index" class="userbtn" style="margin-left:10px;margin-bottom:10px;">
                    {{item.name}}
                </el-tag>
                <el-tag v-for="(item,index) in deployObjputong" :key="index" class="userbtn" style="margin-left:10px;margin-bottom:10px;" @close="handleCloseputong(index)" :closable="!chack_buleputong">
                    {{item.name}}
                </el-tag>
          </div>
            ...
            <ul style="text-align: left;margin-left: 48px;" class="attachment-wrap">
              <li v-for="item in selectedFileList" class="attachemnt-list-item">
                <p class="attachemnt-list-doc" style="float: none;">
                  <span class="doc-name" @click="$utils.handleUploadFilePreview(item, {projectId:projectId,sourceName:'主工作台-审批',projectName:projectName})" :title="item.docName">{{item.docName}}</span>
                  <span>
<!--                      <el-button v-if="item.addSource!==1" type="text" @click="dowanFs(item)">下载</el-button>-->
    ...
    <el-dialog title="审批催办" align="left" :close-on-click-modal="false" :visible.sync="dialogVisible2" center width="480px">
      <div>
              ...
              <ul style="text-align:left;">
                ...
                <div v-if="attachmentShow" class=''>
                    <div style="font-weight:400;text-align:left;" class="clearFix">
                        ...
                        <button  @click="downLoadFilesfujian" class="fr down-load-btn" style="margin-right:30px; ">下载</button>
                    </div>
                    ...
                    <li class='attach-files' style="max-height: 210px;overflow: auto;">
                        ...
                        <span style="margin-right: 3px;" @click="downLoadFiles(item, ind)">下载</span> -->
                            ...
                            <el-checkbox-group class="approval-group" v-model="annexfujian" @change="handleCheckedCitiesChangefujian">
                                <ul>
                                     <el-checkbox  class="attachemnt-list-item select-box approval-upload" :key="item.fileId" :label="item.fileId">
                                        <span class="doc-name ellipsis1"  @click="previewFn(item)" :title="item.fileName">{{item.fileName}}</span>
                                    </el-checkbox>
                                </ul>
                            </el-checkbox-group>
                        </div>
                    ...
                    <span class="result addleft" v-else>{{item.status}}</span>
                     ...
                     <div class="dialogs_files"  v-if='item.commentAttachmentshow'>
                      <div class="clearFix">
                            ...
                            <button  @click="downLoadFilesfujsecond" class="fr" style="margin-right:30px;">下载</button>
                      </div>
...
<!--    <upload-add-doc v-if="addDoc" ref="uploadRef" :uploadDocAddIsShow="uploadDocAddIsShow" @sendValueToParent="sendValueToParentFn" @docUpload="docUploadFn" @docUploadAllsucess="docUploadAllsucess" :uploadParamData="uploadParamData"></upload-add-doc>-->
    <relation-Add-Doc v-if="relafag" @clearstate="clears" :getProjectId="queryDocProjectId" @elationUp="elationUpFn"></relation-Add-Doc>
              ...
              <ul style="text-align: left;" class="attachment-wrap">
                <li v-for="item in selectedFileMeeting" class="attachemnt-list-item">
                  <p class="attachemnt-list-doc">
                    <span class="doc-name" @click="$utils.handleUploadFilePreview(item, {projectId:queryDocProjectId,sourceName:'主工作台-会议',projectName:projectName})" :title="item.docName">{{item.docName}}</span>
                    <span>
<!--                      <el-button v-if="item.addSource!==1" type="text" @click="dowanFs(item)">下载</el-button>-->
...
<!--                      <el-button v-if="item.addSource!==1" type="text" @click="dowanFs(item)">下载</el-button>-->
    ...
    <el-dialog title="取消投票关联提醒" :close-on-click-modal="false" :visible.sync="QXgCvote" width="550px">
      <div class="exit_container">
        <el-scrollbar style="height:100%">
          ...
          <p style="padding:30px 0;text-align:center" v-if="Assd_Cvote.length<=0">没有关联的投票</p>
        </el-scrollbar>
      </div>
    </el-dialog>
              ...
              <ul style="text-align: left;height:60px;">
                ...
                <el-scrollbar style="height:100%">
                  <li v-for="item in selectedFileVote" class="attachemnt-list-item">
                    <p class="attachemnt-list-doc">
                      <span class="doc-name" @click="$utils.handleUploadFilePreview(item, {projectId:queryDocProjectId,sourceName:'主工作台-投票',projectName:projectName})" :title="item.docName">{{item.docName}}</span>
                      <span>
    ...
    <find-deptuser :findFlagShow.sync="findFlag" v-on:findAllUser="findAllUser"
                  :findState="findState" :checkState="checkState"></find-deptuser>
   ...
   <approval-record
    ...
    <find-deptuser :findFlagShow.sync="findFlagputong"
                  @findAllUser="findAllUserputong"
                  :findState="{ state: 0 }"
                  :checkState="{ state: 2 }"
                  :findUserObj="findUserObjputong"></find-deptuser>
  </div>
</template>
<script>
//选择人员
...
export default {
  ...
  methods: {
    handleCloseputong(index) {
      this.deployObjputong.splice(index, 1)
    },
    findAllUserputong(data) {
      ...
      this.deployObjputong = data.filter(v => !v.originData)
    },
    //  选择抄送人
    optPrinFnputong() {
      this.findFlagputong = true;
      let originList = this.colyObjputong.map(v => {
        v.originData = true
        return v
      })
      this.findUserObjputong = [...originList,...this.deployObjputong]
    },
    ...
    handleCheckAllChangefujsecond(val) {
        ...
        this.annexfujsecond = val ? this.approvelistArray.map(v => v.fileId) : [];
    ...
    handleCheckedCitiesChangefujsecond(value) {
        console.log(value, 66)
        let checkedCount = value.length;
        this.checkAllfujsecond = checkedCount ===this.approvelistArray.length;
        this.isIndeterminatefujsecond = checkedCount > 0 && checkedCount < this.approvelistArray.length;
    },
        downLoadFilesaaa () {
        if (this.annex == ""){
    ...
    downLoadFilesfujsecond() {
        ...
        this.annexfujsecond.forEach(item => {
            console.log(item)
            this.approvelistArray.forEach(it => {
                console.log(it)
            if (item == it.fileId && it.delFlag != 1) {
                if (this.$store.state.isPC) {
                    uploadData.push({
        ...
        if(uploadData.length == 1 ){
            if (this.$store.state.isPC) {
                this.$store.commit("pcOtherDownload", {
                    docName : uploadData[0].fileName,
                    docId :uploadData[0].fileId
                });
            } else {
                this.$store.commit("download", uploadData);
            }
        }else{
            //console.log(22)
        var data={
    ...
    findAllUser (data) {
      ...
      if (this.zhongnum == 1) {
        ...
        if (this.aeurv !== "") {
          // this.meeting.Voterusids = Array.from(new Set(this.aeurv));
    ...
    elationUpmanscFn (uploadData) {
      ...
      if (this.uploadnum == 1) {
        ...
        let obj = {};
        this.shagreatlist = this.shagreatlist.reduce((cur, next) => {
        ...
        let obj = {};
        _this.meeting.Relation = _this.meeting.Relation.reduce((cur, next) => {
          obj[next.id] ? "" : obj[next.id] = true && cur.push(next);
          return cur;
        }, []) //
      } else {
        ...
        let obj = {};
        _this.msgObj.Relation = _this.msgObj.Relation.reduce((cur, next) => {
          obj[next.id] ? "" : obj[next.id] = true && cur.push(next);
          return cur;
        }, []) //设置c
      }
    ...
    handleEdit (index, row) {
      ...
      // console.log(row.scope.row.categoryName)
      ...
      this.queryDocProjectId = row.projectId;
      this.mancDocProjectId = row.projectId;
    ...
    approved (item) {
      ...
      this.$post(urls, datas)
        .then((response) => {
          ...
          if (_this.approved_memo.list && _this.approved_memo.list.length) {
            ...
            for(var i = 0; i<_this.approved_memo.list.length; i++){
                console.log(_this.approved_memo.list[i].commentAttachment)
                this.approvelistArray = _this.approved_memo.list[i].commentAttachment
            }
          }
    ...
    previewFn(itemValue) { //文件预览 itemValue接收当前点击文件的数据
        ...
        var previewData = {
            ...
            sourceName: this.itemState[3] === '待办事项' ? '主工作台-待办事项-审批-审批记录' : this.itemState[1].source
        }
        this.$store.commit('previewAllFn',previewData)
      },
    // 审核记录下载附件
    downLoadFiles (item, index) {
      console.log(item, '下载232323232', item.fileName, item.fileId)
      if(this.$store.state.isPC) {
        this.$store.commit("pcOtherDownload",{
          docId: item.fileId,
          docName: item.fileName
        ...
        this.$store.commit("download", [{ name: item.fileName, id: item.fileId }]);
      }
    },
    ...
    reminder (item) {
      ...
      this.$post(urls, datas)
        .then((response) => {
          ...
          _this.textaress =
            item.username + "提醒您审批他的" + item.categoryName
    ...
    fsgj () {
      ...
      this.meetcheckList = Array.from(new Set(this.meetcheckList));
    },
    duaoxuan () {
...
.approval-upload .el-checkbox__label >span{
