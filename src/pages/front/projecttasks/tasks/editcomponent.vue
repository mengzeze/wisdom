<template>
  <div>
    ...
    >
      <div class="clearFix">
        <div
          class="fl taskaddinfo"
          :style="checked ? 'height:610px;' : 'height:469px;'"
        >
          <el-form :model="form" :ref="form">
            ...
            <div
              class="earthquake"
              style="position: relative;margin-left: -33px;"
            >
              <el-checkbox class="tasknpl" v-model="checkedget"></el-checkbox>
              <div
                class="block flex a-center"
                :class="!checkedget ? 'disabledinfo' : ''"
              >
                <span
                  class="demonstrations"
                  style="margin-left: 53px;padding-right: 5px;"
                  >起止时间</span
                >
                <el-date-picker
                  size="small"
                  @change="startpicker()"
                  :disabled="checked ? isCustomdisableis : false"
                  style="width: 167px;"
              ...
              <div class="block" :class="!checkedget ? 'disabledinfo' : ''">
            ...
            <div class="Customtime" v-if="Customdisableis">
              <div>
                <span
                  class="demonstrations"
                  style="margin-left: 53px;padding-right: 17px;"
                  >自定义时间</span
                >
                <el-select
            ...
            <div style="position: relative">
              <el-checkbox class="tasknpl2" v-model="checkedget2"></el-checkbox>
              <el-form-item
                label="重复周期"
                :label-width="formLabelWidth"
                :class="!checkedget2 ? 'disabledinfo' : ''"
                style="margin-bottom: 20px;margin-top: 21px;margin-left: 16px"
              >
                <el-select
            ...
            <div style="position: relative">
              <el-checkbox class="tasknpl2" v-model="checkedget3"></el-checkbox>
              <el-form-item
                label="任务提醒"
                :class="!checkedget3 ? 'disabledinfo' : ''"
            ...
            <div style="position: relative">
              <el-checkbox class="tasknpl3" v-model="checkedget4"></el-checkbox>
              <el-form-item
                label="优先级"
                :class="!checkedget4 ? 'disabledinfo' : ''"
                style="margin-left: 16px"
                :label-width="formLabelWidth"
              >
                <div v-if="status1">
            ...
            <!-- <el-form-item label="发行人" :label-width="formLabelWidth">
              <div>
                <el-checkbox
                  ...
                  v-model="Remind"
                >提醒发行人</el-checkbox>
                <i v-if="Remind" @click="addtask" class="iconfont webicon308 color-primary"></i>
              </div>
              <div>
                <el-tag
          ...
          <div
            ...
            style="text-align: right;padding-top: 49px;"
          >
...
export default {
  ...
  watch: {
    ...
    changeWindoweditor(val) {
      this.value12 = "普通";
      if (!val) {
        this.$emit("seteditcomponent", val);
        this.checkedget = false;
        this.checkedget2 = false;
        this.checkedget3 = false;
        this.checkedget4 = false;
      }
    },
  ...
  methods: {
    ...
    shelterpars() {
      ...
      this.flags = true;
      this.findUserObj = this.executors;
    },
    shelterGlobal() {
    ...
    saveTasks(Switch_to) {
      ...
      var data = {
        ...
        data: {
          ...
          name: this.name,
          // remindIssuer: 0,
          idSet: this.editcomponentdata.map(v => v.id),
          priority: this.label12,
          ...
          endDayType: startDayType, // "结束时间的类型（0/1：自然日/工作日）"
          taskRepetitionPeriod: this.taksTplRepetitionPeriod,
          taskRepetitionPeriodFlag: this.checkedget2,
          timeFlag: this.checkedget,
          customizedFlag: this.checked,
          priorityFlag: this.checkedget4,
          remindFlag: this.checkedget3
        }
      };
      this.$post("info/task/updateTasks", data)
        .then(response => {
          if (response.code == 0 || response.code == -3517) {
            var m;
            if (
              response.data.failNum == null &&
              this.editcomponentdata.length == 1
            ) {
              m = "成功";
            } else {
              if (response.data.failNum == null) {
                ...
                m = `${response.data.successNum}个任务编辑成功，${response.data.failNum}个任务编辑失败`;
              }
            }
