  ...
  <div id="taskinfossssss">
    ...
    >
      ...
      <div class="clearfix" style="padding-left: 64px;" @click.stop="the_alls">
        ...
        <div
          class="fl"
          style="height: 660px;border-right: 1px solid rgb(232, 232, 232);text-align: left;width: 572px;margin-left: -21px;overflow: auto;"
        >
          <!-- baseMessage -->
          <el-scrollbar>
            <el-form :model="form" :ref="form">
              ...
              <div class="Customtime" v-if="Customdisableis">
                <div :class="[forbiddenis == 4 ? 'notclick' : '']">
                  <span
                    class="demonstrations"
                    style="margin-left: 68px;padding-right: 5px;"
                    >自定义时间</span
                  >
                  <el-select
                ...
                <div :class="[forbiddenis == 4 ? 'notclick' : '']">
                  <div class="seletime">
                    ...
                    <div
                      style="float: right;margin-right: 82px;"
                      :class="[forbiddenis == 4 ? 'notclick' : '']"
                    >
                      <el-select
...
export default {
  props: ["flagdetail", "flagdetaildatas", "ids", "status", "dayj"],
  components: {
    relatedFile,
    associatedContent,
    surveyTool,
  ...
  methods: {
    saveTasksrl() {
      ...
      var _this = this;
      this.$post("/info/task/claimTask", data)
        .then(response => {
          if (response.code == 0) {
            _this.$message({
              type: "success",
    ...
    setchecktask(state) {
      this.forbiddenis = state ? 3 : 4;
      this.$post(state ? "/info/task/completeTask" : "/info/task/toUnderway", {
        token: this.token,
        userId: this.userId,
        data: {
          id: this.flagdetaildatas.id
        }
      })
        .then(response => {
          this.setstate = true;
          this.$emit("update", this.flagdetaildatas);
          //  this.clearUpdata()
          if (response.code == 0) {
            this.cishu = !this.cishu;
    ...
    forbidden(val) {
      ...
      //         data:{
      //         id:val.id}}
      //  this.$post('/info/task/getTaskDetailsByid', data).then((response)=> {
      this.forbiddenis = 4;
      this.titeforbidden = "待认领（不可编辑）";
      this.forbiddenisinfo = true;
      // }).catch(function(error) {})
    },
    // 获取通知方式
    getPromptType() {
    ...
    comonm(val) {
      ...
      this.txdisabled = val.remindIssuer == 0 ? true : false;
      this.Remind = val.remindIssuer == 1 ? true : false;
    ...
    gx(item) {
      ...
      this.$post("/info/task/getTaskDetailsByid", data)
        .then(response => {
          ...
          if (
            (this.repetitionvalue == "不重复" || this.repetitionvalue == 0) &&
            (val.remindType == 0 || val.remindVay == null)
          ) {
            this.isShowRemind = false;
          } else {
            this.isShowRemind = true;
    ...
    shelterpars() {
      ...
      // this.executor.forEach(v => {v.originData = true})
