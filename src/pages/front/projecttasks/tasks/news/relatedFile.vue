<template>
  <div class="projecjournal">
    <div class="surveyTool" style="width: 463px;    margin-top: 3px;">
      <p class="addHref">
         <el-button type="text" @click="addSurvey = true">
          <span>
            <div class="riheader">
              <el-dropdown>
                <span class="el-dropdown-link color-primary">添加附件</span>
      ...
      <ul class="surveyDetail" style="height:427px">
        <el-scrollbar style="height:100%">
          <div style="width: 100%;margin-top: 1%;" v-for="item in listdatasss">
            <span
              v-if="item.delFlag == 1"
              style="text-decoration:line-through;float: left;padding-left: 11px;"
            >{{item.name}}</span>
            <span
              style="float: left;padding-top: 2%;padding-left: 11px;"
              @click="Flags(item)"
              v-else
            >{{item.name}}</span>
            <div style="float: right;padding-right: 16px;">
              <span v-if="item.delFlag == 0 || item.delFlag == null">
                <el-button disabled type="text">删除</el-button>
              </span>
              <span v-if="item.delFlag == 0 || item.delFlag == null" style="padding-right: 5px;">
                <el-button disabled type="text">下载</el-button>
              </span>
            </div>
...
export default {
  ...
  methods: {
    ...
    delects(item) {
      this.$confirm("是否删除" + item.name, "是否继续?", "提示", {
        ...
        .then(() => {
          ...
          var _this = this;
          this.$post("/info/task/delTaskFile", data)
            .then((response)=> {
              if (response.code == 0) {
                _this.$message({
    ...
    docUploadFn(uploadData) {
      ...
      // console.log(shagchaunlist)
      // this.shagchaunlist=this.shagchaunlist.concat(shagchaunlist)
    ...
    addjoumal() {
      ...
      if (this.ri_data.type == "") {
        this.$message.warning("日志类型状态内容不能为空");
        return;
      }
