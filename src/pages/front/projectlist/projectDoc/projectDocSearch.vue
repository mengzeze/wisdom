    ...
    <div class="project-doc-search">
        <div class="project-doc-top">
            ...
            <el-breadcrumb separator="/">
                ...
                <el-breadcrumb-item>项目文档搜索结果</el-breadcrumb-item>
            </el-breadcrumb>
            <div class="clearfix">
                <el-button icon="el-icon-back" circle @click="back" class="back_btn fl"></el-button>
                <p class="head fl">项目文档搜索结果</p>
            </div>
        </div>
        <div class="project-doc-con">
            <div class="search-box clearfix">
                <div :class="[{'search-item-box-max': moreSearchAutoH},{'search-item-box-min': !packSearchVisible},'search-item-box','fl']" v-on:keyup.enter="queryDocBtn">
                    <el-button type="text" class="more-stage bgcolor-primary-hover" @click="showAllBtn(1)" v-if="!packSearchVisible  && moreSearchBtnVisible">
                        更多
                        <i class="iconfont jiantou"></i>
                    </el-button>
                    <el-button type="text" class="more-stage bgcolor-primary-hover" @click="packUpBtn(1)" v-if="packSearchVisible">
                        收起
                    ...
                    <div class="form_box">
                        <el-form ref="form" :model="mySearch.searchList" label-width="100px" class="clearfix">
                            <el-form-item label="标题关键词：">
                                <el-input class="q-308" v-model="mySearch.searchList.titleKey" placeholder="包含以下全部关键词（以空格区分）" maxlength="160" ></el-input>
                            </el-form-item>
                            <el-form-item label="内容关键词：">
                                <el-input class="q-308" v-model="mySearch.searchList.conKey" placeholder="包含以下全部关键词（以空格区分）" maxlength="160"></el-input>
                            </el-form-item>
                            <el-form-item label="项目阶段：">
                                <el-select class="q-240" v-model="mySearch.searchList.proStage" placeholder="请选择项目阶段" clearable @change="changeProStage">
                                <el-option v-for="(item, index) in mySearch.proStageList" :key="index" :label="item.name" :value="item.id"></el-option>
                                </el-select>
                            </el-form-item>
                            <el-form-item label="文件类型：">
                                <multiple-choice ref="docTypeEle" @changeData="fileTypesChangeData" :defaultSelectedValues='mySearch.searchList.fileTypesSel' :selectData="mySearch.fileTypes" :width="196"></multiple-choice>
                            </el-form-item>
                            <el-form-item label="文件状态：">
                                <multiple-choice ref="docStateEle" @changeData="fileStateChangeData" :defaultSelectedValues='mySearch.searchList.fileStateSel' :selectData="mySearch.fileState" :width="196"></multiple-choice>
                            </el-form-item>
                            <el-form-item label="目录状态：">
                                <multiple-choice ref="dirStateEle" @changeData="dirStateChangeData" :defaultSelectedValues='mySearch.searchList.dirStateSel' :selectData="mySearch.dirState" :width="196"></multiple-choice>
                            </el-form-item>
                            <el-form-item label="修改日期：">
                            <date-range :date-start.sync="mySearch.searchList.starTime"
                                        :date-end.sync="mySearch.searchList.endTime"
                                        format="yyyy-MM-dd HH:mm:ss"></date-range>
                            </el-form-item>
                            <el-button size="medium" type="primary" icon="el-icon-search" @click="queryDocBtn">查询</el-button>
                            <el-button size="medium" icon="el-icon-refresh" @click="reset">重置</el-button>
                        </el-form>
                    </div>
                </div>
                <div class="search-btn-box fr" v-if="!packSearchVisible">
                    <el-button  type="primary" icon="el-icon-search" @click="queryDocBtn">查询</el-button>
            ...
            <div :class="[{'pro-doc-operation-max':moreHandleAutoH},{'pro-doc-operation2':!moreHandleAutoH},'pro-doc-operation','clearfix']">
                <el-button type="text" class="more-stage bgcolor-primary-hover" @click="showAllBtn(2)" v-if="!packHandleVisible  && moreHandleBtnVisible">
                    更多
                    <i class="iconfont jiantou"></i>
                </el-button>
                <el-button type="text" class="more-stage bgcolor-primary-hover" @click="packUpBtn(2)" v-if="packHandleVisible">
                    收起
                    <i class="iconfont jiantou_shang"></i>
                </el-button>
            ...
            <div class="project-doc-list">
                 <el-table
                    ...
                    @selection-change="handleSelectionChange" :height="tableHeight" @sort-change="tableSortChange">
                    <el-table-column type="selection" width="55"></el-table-column>
                    <el-table-column label="文件名" prop="docName" min-width="560" sortable="custom">
                        <template slot-scope="scope">
                            <el-col :span="15" class="name_col clearfix" style="position:relative" :title="scope.row.docName">
                                    ...
                                    <div class="fl name_col_name">
                                        <div :class="[{noIcon:scope.row.auditStatus == null && !scope.row.notes && !scope.row.lockState && !scope.row.record && !scope.row.reminder && scope.row.linkList == null && scope.row.paperLabelList == null},'table-doc-name']" v-html="scope.row.highlightName"  :title="scope.row.docName">
                                        </div>
                    ...
                    <el-table-column label="所在阶段" prop="stageName" min-width="200">
                    </el-table-column>
                    <el-table-column label="修改时间" prop="updateTime" min-width="160" sortable="custom">
                        <template slot-scope="scope">
                            <p v-if="scope.row.docType == 0">{{scope.row.updateTime}}</p>
                        </template>
                    </el-table-column>
                    <el-table-column label="大小" prop="size" min-width="140" sortable="custom">
                    </el-table-column>
        ...
        <file-version :versionIsShow="versionIsShow"
                    ...
                    @queryDoc="versionUpdate"></file-version>
        <!-- 设为底稿 -->
        <doc-relation
            ref="docRelationObj"
        ...
        <doc-exam
            ...
            @sendToParentQueryDoc="queryDocWay"
...
export default {
    ...
    created() {
        ...
        this.searchedTerm = this.$store.state.proDocSearchList;//获取传过来的查询条件
        this.projectData = this.$store.state.proDocSearchProData;
        console.log('项目数据',this.searchedTerm)
        ...
        this.mySearch.searchList.dirStateSel = this.searchedTerm.searchList.dirStateSel;// 目录状态检索条件
        this.mySearch.searchList.fileStateSel = this.searchedTerm.searchList.fileStateSel; // 文件状态检索条件
        this.mySearch.searchList.fileTypesSel = this.searchedTerm.searchList.fileTypesSel;//文件类型检索条件
    ...
    watch: {
        ...
        selectedData:{
            handler(){
                console.log('数据',this.selectedData)
    ...
    methods: {
      ...
      fileApprove (procTypeId) {
        ...
        displaySelected() {
            this.$select.handleDisplaySelection(1,this.publicData.projectId,this.publicData.projectId,this.docInitList).then((res=[])=>{
                console.log('handleDisplaySelection-doc', res)
                this.docInitList.forEach(t=>{
                    let idx = res.findIndex(m => m === t.id)
                    this.$refs.multipleTable.toggleRowSelection(t, idx>-1);
                })
            })
        },
        ...
        relatedSelect(res, selection){
            console.log('relatedSelected', res, selection)
            res.forEach(v => {
                let idx = this.docInitList.findIndex(m => m.id === v)
                console.log(idx)
                this.$nextTick(() => {
                   idx>-1 && this.$refs.multipleTable.toggleRowSelection(this.docInitList[idx], selection);
                })
            })
        },
        exportBtn(){
        ...
        dirStateChangeData (arr) {
            this.mySearch.searchList.dirStateSel = arr.map(v => v.value);
        },
        // 文件状态选择
        fileStateChangeData (arr) {
            this.mySearch.searchList.fileStateSel = arr.map(v => v.value);
        },
        // 文件类型选择
        fileTypesChangeData (arr) {
            this.mySearch.searchList.fileTypesSel = arr.map(v => v.value);
        },
        // 点击全选复选框
        handleSelectAll(selection){
            selection.length
                ? this.$select.handleFileSelect(1,this.publicData.projectId, this.publicData.projectId, selection)
                : this.$select.handleCancel(1,this.publicData.projectId, this.publicData.projectId, this.docInitList, this.displaySelected)
        },
        // 点击单个复选框
        handleSelectDoc(selection,row){
            this.$select.fileSelect(1,this.publicData.projectId, this.publicData.projectId, selection,row, this.displaySelected)
        },
        selectedDocList(){//已选文件列表
            ...
            this.$store.commit('proDocSearchFn', this.mySearch)
            this.$store.commit('proDocSearchProFn', this.publicData)
            this.$router.push({path: '/proDocSelectList',query:{projectData: JSON.stringify(this.publicData),fromItem:1}});
        },
        // 取消选择
        clearSelection(type) {
            console.log('走了取消状态')
        ...
        //当前页面查询
        queryDoc() {
            console.log('22222222222222222222222日志',this.publicData.processName)
            let vm = this;
            this.bread_crumbs = [{title: "根目录", parentId: 0}];//路径
            let startTime = this.dateToMs(this.mySearch.searchList.starTime) || '';
            let endTime = this.dateToMs(this.mySearch.searchList.endTime) + 86400000 || '';
            let auditProjectIds = [];
            if(this.mySearch.searchList.proStage != ''){
                auditProjectIds.push(this.mySearch.searchList.proStage);
            }
            let searchStageName = []//记录日志要用到的阶段名称
            if(this.mySearch.searchList.proStage != ''){
                this.mySearch.proStageList.forEach(v => {
                    if (v.id == this.mySearch.searchList.proStage) {
                        searchStageName.push(v.name);
                    }
                })
            }
            let idSet = [];//选中的文件+文件夹id集合
            let seleDocIds = [];//选中文件的id集合
            let seleDirIds = [];
            let selectData = this.$store.state.selectionProjectDoc;
            let curProData = {};
            selectData.forEach(v => {
            ...
            this.selectedDoc = curProData.file || [];
            this.selectedDir = curProData.dir || [];
            this.selectedDoc.map(item=>{
                seleDocIds.push(item.id)
            })
            this.selectedDir.map(item=>{
                seleDirIds.push(item.id)
            })
            this.mySearch.searchList.fileStateSel.find(n=>n==1) != undefined && (idSet = idSet.concat(seleDocIds));
            this.mySearch.searchList.dirStateSel.find(n=>n==3) != undefined && (idSet = idSet.concat(seleDirIds));
            let data = {
                token: this.token,
                userId: this.userId,
                ...
                data: {
                    ...
                    suffixList: this.mySearch.searchList.fileTypesSel, //文件类型（后缀名）
                    fileStatus: this.mySearch.searchList.fileStateSel,
                    dirStatus: this.mySearch.searchList.dirStateSel,
                    idSet: idSet,
        ...
        termQueryDoc() {
            ...
            let data = {
                ...
                "data":{
                    ...
                    "beginTime": startTime,//文件创建开始时间
                    "endTime": endTime,//文件创建结束时间
                    suffixList: this.searchedTerm.searchList.fileTypesSel, //文件类型（后缀名）
                    fileStatus: this.searchedTerm.searchList.fileStateSel,
                    dirStatus: this.searchedTerm.searchList.dirStateSel,
                    idSet: [],
                    orderFlag: this.orderFlag,
                    "isSearch": true,
                    auditProjectNames: [this.publicData.processName],
                }
            }
            this.$post('/doc/project/queryFileByCondition', data).then((response)=> {
                if(vm.success_code == response.code) {
        ...
        //回收站
        goRecovery() {
            this.$router.push({path:'/proDocRecovery',query:{projectId: this.projectId}})
        },
        //分页
        ...
        closeremind(){
            ...
            this.$post('/doc/project/closeReminder', data).then((response)=> {
                if(response.code == _this.success_code){
                    ...
                    console.log(this.isSearch, 'isSearch')
