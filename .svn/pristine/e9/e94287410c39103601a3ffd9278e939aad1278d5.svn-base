<template>
    <div>
        <div class="surveyTool" style="width: 463px;margin-top: 2px;">
            <p class="addHref">
                <el-button type="text" class="color-primary" disabled><i class="el-icon-plus"></i><span>添加调查工具</span></el-button>
            </p>
            <ul class="surveyDetail" style="height:427px">
                <el-scrollbar style="height:100%">
                <!-- <li class="clearfix surveyLi;"  v-for="(item,index) in datasurvey" :key=index>
                    <span class="fl"><a target="_blank" :href="item.url">{{item.name}}</a></span>
                    <span>{{item.description}}</span>
                    <em class="fr" @click="deletes(item)">删除</em>
                    <em class="fr" @click="surveyEdit(item)">编辑</em>
                </li> -->
                 <div style="width: 100%;margin-top: 1%;" v-for="item in datasurvey" >
                    <span style="float: left;padding-top: 2%;padding-left: 11px;width: 140px;
                    overflow: hidden;height: 23px;white-space: nowrap;text-overflow: ellipsis">
                    {{item.name}}</span>
                     <span style="float: left;margin-top: 9px;padding-left: 11%;color: #999999;width: 145px;height: 23px;
                    overflow: hidden;white-space: nowrap;text-overflow: ellipsis">{{item.description}}</span>
                    <div style="float: right;padding-right: 16px;">
                    <span><el-button disabled type="text">编辑</el-button></span>
                    <span ><el-button disabled style="padding-right: 5px;" type="text">删除</el-button></span>
                    </div>
        ...
        <el-dialog
            ...
            width="500px" :append-to-body='true'>
            <span>
                <div>
                    <el-form :label-position="labelPosition" label-width="80px" ref="formadd" :model="formadd">
                        <el-form-item label="">
                            <span style="float:left;margin-left: -20%;"><i style="color: red;">*</i> 名称:</span>
                            <span style="float:left;width: 100%;"> <el-input maxlength="50" v-model="formadd.name"></el-input></span>
                        </el-form-item>
                        <el-form-item label="">
                            <span style="float:left;margin-left: -20%;"><i style="color: red;">*</i>工具链接:</span>
                            <el-input maxlength="500" v-model="formadd.url"></el-input>
                        </el-form-item>
                        <el-form-item label="">
                           <span style="float:left;margin-left: -20%;"><i style="color: red;">*</i>描述:</span>
                            <el-input type="textarea"   resize='none' maxlength="2000" v-model="formadd.description"></el-input>
                        </el-form-item>
                    </el-form>
                </div>
            </span>
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="addSurveysave(formadd)">保 存</el-button>
            </span>
        </el-dialog>
        <!-- 编辑调查工具 -->
        <el-dialog
            title="编辑调查工具"
            :visible.sync="editorSurvey"
            :close-on-click-modal="false"
            width="500px" :append-to-body='true'>
            <span>
                <div>
                    <el-form :label-position="labelPosition" label-width="80px" ref="formLabEdit" :model="formLabEdit">
            ...
            <span slot="footer" class="dialog-footer">
                ...
                <el-button type="primary" @click="surveyModefl">保 存</el-button>
            </span>
        </el-dialog>
        <!-- 删除 -->
        <el-dialog
            ...
            width="15%"
            :close-on-click-modal="false"
            :before-close="canceldelete" :append-to-body='true'>
            <span>
                确定要删除吗？
            </span>
            <span slot="footer" class="dialog-footer">
...
export default {
    ...
    methods:{
        ...
        //新增调查工具
        addSurveysave(){
            if(this.formadd.name == '' || this.formadd.url =='' ||  this.formadd.description ==''){
                this.$message({
                type: 'info',
                message: '请填写完整'
                });
            ...
            var thisObjet = this;
            this.$post('/info/task/saveTaskTool', datas).then((response)=> {
                if(response.code == 0){
        ...
        surveylist(ids){
            ...
            this.$post('/info/task/getTaskToolByTaskid', dataList).then((response)=> {
        ...
        surveyModefl(){
            ...
            var _this = this;
            this.$post('/info/task/updateTaskTool', dataList).then((response)=> {
        ...
        deletes(item){
            ...
            console.log(data)
            var _this=this
            this.$post('/info/task/delTaskTool', data).then((response)=> {
                if(response.code == 0){
                    _this.$message({
                    type: 'success',
        ...
        deleteok(){
            ...
            var _this = this;
            this.$post('/info/task/delTaskTool', datas).then((response)=> {
...
.attentionMatters, .rightShelter .relatedFile, .rightShelter .surveyTool, .rightShelter .associatedContent {
    margin-top: 3px;
}
