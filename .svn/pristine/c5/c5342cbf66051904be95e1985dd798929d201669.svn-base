...
// const  URL_UPLOAD = "http://shrd.f3322.org:1066";
export class wisdom_doc {
  ...
  fileUploadedComplete(up, file, rfsId, hashCode) {
    ...
    axios.post('/doc/insert', arg, {
      ...
      this.param.FileUploaded(file, response.data.data.docId, response.data.data.docName, response.data.data.suffix);
        // this.uploader.destroy();
    })).catch(error => {
      this.param.error(error);
    });
