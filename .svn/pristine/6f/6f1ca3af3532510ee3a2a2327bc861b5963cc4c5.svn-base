<template>
  <div class="content sponsor">
    <el-dialog :title="procNamedos+'审批'"
      ...
      <i style="display: inline-block;width: 800px;background: #cccc;height: 1px;margin-top: -23px;float: left;margin-left: -26px;"></i>
      <div class="project">
        <!-- <p></p> -->
          ...
          <ul>
            ...
            <li style="margin-left:12%;">
              <p style="border: none;">
                ...
                <i>项目编码：</i>
                <span>
                  <el-input size="mini" v-model="xmbhsss" disabled placeholder="请选择项目编码" style="width:250px;"></el-input>
                </span>
              </p>
            </li>
             <li style="width: 50%;" v-show="isMultiDept.isMultiDept">
              <p style="border: none;">
                ...
                <i>发起人所属部门：</i>
                <span>
                  <el-select v-model="isMultiDeptvalue" size="mini" @change="procTypeIds" filterable placeholder="请选择" style="width:250px;">
                    <el-option v-for="item in isMultiDept.deptList" :key="item.id" :label="item.name" :value="item.id"></el-option>
                  </el-select>
                </span>
              </p>
            </li>
            <li v-for="(item,index) in inputdata.form" v-if="plain" :key="item">
              <p style="border: none;" v-if="item.type.name == 'date'">
                <i style="color:red;padding-right: 3px;">*</i>
                <i>{{item.name}}：</i>
                <span>
                  <el-date-picker v-model="list[index]" type="date" placeholder="选择日期" @focus="$utils.handleTimeFocus"></el-date-picker>
                </span>
              </p>
            ...
            <!-- 6 -->
            <li class="xm_messageul" v-for="(item,index) in inputdata.form" style="padding-bottom: 15px;"   :class="sstt[index]" v-if="plain2" :key="item">
              <p class="clearFix" style="border: none;margin-left: 1px;" v-if="item.id == 'reason' || item.id == 'description' || item.type.name == 'string'">
                ...
                <span style="width: 246px;margin-left:5px;">
                  ...
                  <el-input size="mini" type="textarea" resize="none" id="inputs" maxlength="1000" v-model="list[index]" placeholder="请输入内容"></el-input>
                </span>
              </p>
...
<!--            {{sjwj}}-->
<!--          </div>-->
<!--          <p style="float: left;width: 10%;border: none;margin-top: -49px;margin-left:99px;" class="project2span">-->
<!--            <el-popover placement="bottom" width="100" trigger="hover" style="float:left;padding:5px;width:500px;text-align: left;width:98%" align="left">-->
<!--              <el-button width="100" type="text" style="float:left;padding-left:8px;border:none;" @click="uploadingFns">本地上传</el-button>-->
<!--              <el-button v-if="xmwds" width="100" type="text" style="float:left;border:none;" class="cvote_button" @click="upreats">项目文档选择</el-button>-->
<!--              <el-button v-if="xmwds2" width="100" type="text" style="float:left;border:none;" class="cvote_button" @click="upreats2">项目文档选择</el-button>-->
<!--              <el-button width="100" type="text" style="float:left;border:none;" class="cvote_button" @click="upreatdigao(2)" v-if="$utils.m('paper_manage')">底稿选择</el-button>-->
<!--              <el-button slot="reference" style="color:#0061A9" type="text">-->
<!--                <img src="../../../assets/image/filedowns.png" alt style="position:relative;top:2px;margin-right:10px;" />添加文件-->
<!--              </el-button>-->
<!--            </el-popover>-->
<!--          </p>-->
          ...
          <ul class="file_list" style>
              <li v-for="item in selectedFileListCheck" :key="item">
                <p class="attachemnt-list-title">
                  <i class="doc-name" @click="$utils.handleUploadFilePreview(item, queryDocProjectId)" :title="item.docName">{{item.docName}}</i>
        ...
        <div class="upload_file" v-if="seensk">
          ...
          <ul style="text-align: left;max-height: 100px;overflow: auto" class="attachment-wrap">
            <li v-for="item in selectedFileList" class="attachemnt-list-item" :key="item">
              <p class="attachemnt-list-doc">
                <span class="doc-name" @click="$utils.handleUploadFilePreview(item, queryDocProjectId)" :title="item.docName">{{item.docName}}</span>
                <span>
        ...
        <div class="project3 shhj">
          ...
          <div v-show="!MultiDeptvalue">
             <h5 style="margin-left: 37px;margin-top: 26px;font-size: 14px;font-weight: bold;">发起人：<span style="font-weight: 100;">{{faname}}</span></h5>
          <ul class="sp_people">
            ...
            <div id="copypeo">
              <el-scrollbar style="height:100%;">
                <li style="margin-left: 6%;width: 700px;" class="cs_people">
                  <el-tag class v-for="(tag,index) in  inputdatacopyName" :key="index" style="margin-left: 3px;margin-top: 3px;" >{{tag}}</el-tag>
                  <el-tag v-for="(tag,idx) in ches" :key="idx" closable @close="closess(tag)" :type="tag.type" style="margin-left: 3px;margin-top: 3px;" >{{tag.name}}</el-tag>
            ...
            <el-scrollbar style="height: 100%">
                <el-form :model="formInline" class="demo-form-inline"  label-width="100px" style="text-align:left">
                    ...
                    <el-form-item label="接收人员：" class="remindRight">
                        <el-tag class v-for="(tag,index) in  inputdatapprovalName" :key="index" style="margin-left: 3px;margin-top: 3px;" @close="deletePerFnexam(tag,index)" :closable="inputdatapprovalName.length == 1?false:true" >{{tag}}</el-tag>
                ...
                <el-form :model="formInlinefile" class="demo-form-inline"  label-width="100px" style="text-align:left">
                    <el-form-item label="提醒方式：" class="remindRight">
                        <el-checkbox-group v-model="remindWayfile"   @change="bbb"  :min="1">
                            <el-checkbox v-for="way in remindWayGrounpfile" :label="way.label" :key="way.value">{{way.label}}</el-checkbox>
                        </el-checkbox-group>
                    </el-form-item>
                    <el-form-item label="接收人员：" class="remindRight">
                        <el-tag class v-for="(tag,index) in  inputdatapprovalNamefile" :key="index" style="margin-left: 3px;margin-top: 3px;" @close="deletePerFnexamfile(tag,index)" :closable="inputdatapprovalNamefile.length == 1?false:true" >{{tag}}</el-tag>
                        <!-- <el-tag v-for="(tag) in ches" :key="tag.id" :closable="!chack_bule" @close="cdeletePerFnexam(tag)" :type="tag.type" style="margin-left: 3px;margin-top: 3px;" >{{tag.name}}</el-tag> -->
                    </el-form-item>
                    <el-form-item label="提醒内容：" class="remindRight">
                        <el-input type="textarea" cols="40" rows="5" resize="none" v-model.trim="formInlinefile.contentMessage"  maxlength="100" ></el-input>
                        <!-- <el-input type="textarea" cols="40" rows="5" resize="none" v-model.trim="formInline.contentEmial"  maxlength="100" class="remindContent" v-if="contentEmial"></el-input>
                        <el-input type="textarea" cols="40" rows="5" resize="none" v-model.trim="formInline.contentNote"  maxlength="100" class="remindContent" v-if="contentNote"></el-input> -->
                    </el-form-item>
                </el-form>
        ...
        <div class="borrow_middle">
            ...
            <!-- <div class="borrow_middle_chunk_content_remark">
             ...
             <div v-show="initDocExamRoleinfo.key" style="padding-top: 2%;">
                ...
                <el-select size="small" v-model="isMultiDeptvalue" filterable @change="handleInitiatorSection()" placeholder="请选择部门">
                    <el-option
    ...
    <!--项目归档（底稿归档）-->
    <el-dialog :title="titleguidang" :close-on-click-modal="false" class="proPigeonhole_dialog"  @close="proPigeo" :visible.sync="proPigeonholeVisible" width="818px">
        <div class="pigeonhole_middle">
            ...
            <div style="padding-top: 13px" v-show="initDocExamRoleinfo.key">
                <i class="bitians">*</i>发起人所属部门：
                <el-select size="medium" v-model="isMultiDeptvalue" filterable @change="handleInitiatorSection()" placeholder="请选择部门">
                    <el-option
                    v-for="item in isMultiDept.deptList"
                    :key="item.id"
                    :label="item.name"
    ...
    <upload-add-doc v-if="addDoc" ref="uploadRef" :uploadDocAddIsShow="uploadDocAddIsShow" @sendValueToParent="sendValueToParentFn" @docUpload="docUploadFn" @docUploadAllsucess="docUploadAllsucess" :uploadParamData="uploadParamData"></upload-add-doc>
    <relation-Add-Doc v-if="relafag" @clearstate="clears" @elationUp="elationUpFn" :getProjectId="proid"></relation-Add-Doc>
    <manuscript-Add-Doc v-if="relafagmanus" @clearstate="clearsmacuse" :manscProjectId="mancDocProjectId" @elationUpmansc="elationUpmanscFn"></manuscript-Add-Doc>
    <exam-relation-Add-Doc v-if="selectProjectDocIsShow" @clearstates="clearstates" @elationUp="selectProjectDocSure" :getProjectId="proid"></exam-relation-Add-Doc>
...
export default {
  ...
  mounted () {
    this.$Examine()
    this.value = this.$store.state.projectMsg.pro_id
    this.$Mit.watch({
      approve:(target,res)=>{
        let projectMsg = this.$store.state.projectMsg.projectMsg;
        this.queryDocProjectId = this.$store.state.projectMsg.pro_id
        this.projectMessage = projectMsg
        this.value = projectMsg.name
        this.Typesoffinancing = projectMsg.currentImplementStageName
        this.xmbhsss = projectMsg.code;
        this.handleOperate(res)
        // this.procTypeIds()
      }
    })
  ...
  methods: {
          ...
          this.$post("/sys/getNoticeWayConfig")
            ...
            if (this.code.codeNum.SUCCESS == res.code) {
                if(res.data.email == 1) {
                  ...
                  this.remindWayGrounpfile.push({label:"邮件",value:1})
                }
                if(res.data.sms == 1) {
                  this.remindWayGrounp.push({label:"短信",value:2})
                  this.remindWayGrounpfile.push({label:"短信",value:2})
                }
                return;
            }
      ...
      this.uploadDocAddIsShow = false;
        ...
        submitApprove(){
            if(this.isSetRamindchecked == true){
                ...
                console.log(this.examine.projectMsg.projectName)
     ...
     querysShenp(){
      ...
      async remindSetMessage(){
        ...
        return await this.$post(url, data).then((response) => {
            ...
            _this.errorMsg = "";
            let checkedRemindNames = _this.rmindaaa;
            console.log(_this.remindWay)
        ...
        submitApprovefile(){
            if(this.resdata.procTypeName == "底稿归档审批"){
                ...
                if(this.isSetRamindcheckedfile == true){
                   ...
                   this.querysShenpfile()
                    // 站内信模板
                    let projectInfo = this.$store.state.projectMsg.projectMsg.name ? `${this.$store.state.projectMsg.projectMsg.name}项目的`: ''
                    var insideLetter = "【智慧投行】 " + this.$store.state.loginObject.userName+"：" +"提醒您尽快审批关于"+projectInfo+this.resdata.procTypeName +"，可在'我的审批-待我审批中'进行查看，该审批提交时间为 "+this.$moment().format("YYYY-MM-DD HH:mm:ss")
                this.formInlinefile.contentMessage = insideLetter
                    //this.copyNameArrayinfo = [...this.copyNameArray]
                    this.inputdatapprovalNamefile = [...this.approvalNameArray]
      ...
      this.$post(url, data)
        .then((response) => {
          ...
          if (response.code == 0) {
            ...
            // 保存成功，清空草稿数据
            _this.$utils.removeDraft("sponsor", false);
            ...
            _this.proid = "";
            // _this.shagreatlists=[]
            // _this.shagchaunlists=[]
            // _this.shagreatlist=[]
            // _this.shagreatlist=[]
            _this.shagchaunlist2 = [];
            _this.shagchaunlists2 = [];
            _this.shagreatlist = [];
            _this.shagreatlists = [];
    ...
    addpeople () {
      ...
      this.findUserObj = this.inputdatacopyName.map((name,i) => ({name, id: this.inputdatacopyCode[i],label: name, uniqueKey: 'user' + this.inputdatacopyCode[i],originData: true}));
    ...
    async lx(items){
      ...
      await this.examineInit(undefined,(model,instance)=>{
        if(this.dialogVisibles !== true){
          ...
          // this.sjwj = "待审文件";
          this.Regulars = instance.procTypeId; // 判断是否普通审批
    ...
    //处理草稿数据
    handleDraftData () {
      // console.log(this.$refs.selfEditer.getHtmls(),'_____selfEditer')
      // console.log(this.$refs.selfEditer.getText(),'_____selfEditer')
      let data = {
  ...
  watch: {
    ...
    examineInstance(newData){
      this.currentgeneration = newData.type
    },
    examineMessage(newData){
      ...
      if(newData.hint){ this.$message({ type: "info", message: newData.hint }) }
    ...
    // 'inputdata.form': {
    //   handler (newName, oldName) {
    //    console.log(newName[0].name)
    //    console.log(newName[1].name)
    //    for(var i=0;i <newName.length; i++){
    //             if(newName[i].name == "终止原因"){
    //                 console.log(newName[0].name)
    //                 console.log(111)
    //                 this.endreson = true;
    //                 this.endsay = false;
    //             }else if(newName[i].name == "终止说明"){
    //                 console.log(222)
    //                 this.endreson = false;
    //                 this.endsay = true;
    //             }
