...
function locationLogin(){
    window.parent.locationLogin();
}

var opt_user_value = "无代理人";
...
activitiModeler
  ...
  .run(['$rootScope', '$timeout', '$modal', '$translate', '$location', '$window', '$http', '$q',
        function($rootScope, $timeout, $modal, $translate, $location, $window, $http, $q) {
            ...
            function fetchModel(modelId,id) {
                ...
                $http({method: 'post',data:param, url: modelUrl}).
                    success(function (data, status, headers, config) {
                        ...
                        }else if(data.code == -502 || data.code == -513){
                            locationLogin();
                        }else{
            ...
            $rootScope.$on('$includeContentLoaded', function (event) {
	            if (!$rootScope.editorInitialized) {
	                ...
	                var id = EDITOR.UTIL.getParameterByName('id');
	                fetchModel(modelId,id);
