...
export default {
  ...
  methods: {
    ...
    // 投票详情
    voteDetail (item) {
      item.createBy = item.createBy ? item.createBy : item.userId
      this.voteData = item
    ...
    instruction (index, item, state) {
      state == 9 ? item.approvalVo.source = '主工作台-待办事项-审批记录' : item.approvalVo.source = '主工作台-待办事项-审批'
      this.itemState = [state, item.approvalVo, index,'待办事项'];
    },
    updatas (val) {
      if (val) {
        this.currentPage = 1;
