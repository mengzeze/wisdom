  ...
  <div class="content sponsor">
    ...
    <div id="business-wrap" class="business-tree">
      ...
      <div id="business-wrap-content" class="business-content">
        <business-process :business="{}" @operate="handleOperate" :project="businessProject"
      ...
      <div class="project">
        ...
        <div class="project1">
          <div class="project-box">
            <div class="flex">
              <div class="fz-bold project-cont ellipsis1">审批类型：{{typeApproval}}</div>
              <div class="fz-bold project-cont ellipsis1" :title="currentgeneration">业务类型：{{currentgeneration}}</div>
            </div>
            <div class="flex">
              <div class="fz-bold project-cont">项目信息</div>
              <div class="fz-bold project-cont ellipsis1" :title="!Typesoffinancing || Typesoffinancing === '' ? '无' :
                Typesoffinancing">当前阶段：{{!Typesoffinancing || Typesoffinancing === '' ? '无' :
                Typesoffinancing}}
              </div>
            </div>
            <div class="flex mt-5">
              <div class="flex project-cont  a-center">
                <div><i class="require-icon" v-if="regulars != 1">*</i>项目名称：</div>
                <div>
                  <select-lazy
                    v-model="value"
                    size="mini"
              ...
              <div class="flex project-cont  a-center">
                <div><i class="require-icon" v-if="regulars != 1">*</i>项目编码：</div>
                <div>
                  <el-input size="medium" v-model="xmbhsss" disabled placeholder="请选择项目编码"
        ...
        <el-scrollbar style="height: 100%">
          <el-form :model="formInline" class="demo-form-inline" label-width="100px" style="text-align:left">
            <el-form-item label="提醒方式：" class="remindRight">
              <el-checkbox-group v-model="remindWay" :min="1" @change="remindType">
      ...
      <div style="height: calc(100vh - 400px);">
        <el-scrollbar style="height: 100%">
          <el-form :model="formInlinefile" class="demo-form-inline" label-width="100px" style="text-align:left">
            <el-form-item label="提醒方式：" class="remindRight">
              <el-checkbox-group v-model="remindWayfile" @change="remindChange" :min="1">
    ...
    <!--项目归档（底稿归档）-->
    <el-dialog :title="'归档表单'" :close-on-click-modal="false" class="proPigeonhole_dialog" @close="proPigeo"
    ...
    <!-- 选择人员 -->
    <findAllDeptUser @findAllUser="findAllUsers"
  ...
  export default {
    ...
    methods: {
      ...
      async procTypeIds(stateisMultiDept) {
        ...
        if (this.procTypeId == 5 || this.procTypeId == 6) {
          this.disableds = false;
        ...
        if (this.procTypeId == 6) {
          ...
          this.$post("/info/audit/current_project_use", data)
            .then((response) => {
              if (response.code == 0) {
                if (!response.data) {
                  this.$message.warning("当前项目正在进行阶段审核");
                  this.disableds = true;
                  return
                } else {
                  this.disableds = false
      ...
      remindType(value) {
        console.log(value)
        this.remindTypeList = value
      },
      //设置提醒的方法（相当于审批催办）
      setRemaid() {
        console.log(this.remindWay)
        this.remindSetMessage().then(res => {
          var arr = this.remindTypeList;
          var ges,
            arrs = [];
          for (let i = 0; i < arr.length; i++) {
            console.log(arr[i])
      ...
      async remindSetMessagefile() {
        return await this.$post("/sys/getUserList", {
          ...
          this.messagefailedfile = response.data;
          this.errorMsgfile = "";
          let checkedRemindNames = this.remindWayfile;
          for (var i = 0; i < this.messagefailedfile.length; i++) {
            let name = this.messagefailedfile[i].name;
            if (!this.messagefailedfile[i].email && checkedRemindNames.indexOf("邮件") != -1) {
              this.errorMsgfile += name + "的邮箱发送失败；";
            }
            if (!this.messagefailedfile[i].mobile && checkedRemindNames.indexOf("短信") != -1) {
              this.errorMsgfile += name + "的短信发送失败；";
            }
          }
          if (this.errorMsgfile) {
            let errorMsgfile = this.errorMsgfile + "请在个人信息中完善！其余均发送成功";
            // this.$message.error(errorMsg);
            return errorMsgfile;
          }
          return null;
      ...
      // 提醒方式选择
      remindChange(value) {
        this.remindList = value
      },

      //设置提醒的方法（相当于审批催办）
      setRemaidfile() {
        this.remindSetMessagefile().then(res => {
          var arr = this.remindList;
          console.log(arr)
          var ges,
            arrs = [];
          for (let i = 0; i < arr.length; i++) {
      ...
      dialogVisibles1() {
        ...
        if (this.initDocExamRoleinfo.key && this.isMultiDeptvalue == "") {
          this.$message.warning('请选择发起人所属部门')
          return
        }
        ...
        // this.$Mit.approvalSuccessful.code = 200
        // 提交申请
        this.$post("/info/audit/start_process_instance", data)
          .then((response) => {
            this.disableds = false
