<template>
  <div class="upload-doc">
    <el-dialog title="文件上传" :close-on-click-modal="false" :visible.sync="uploadDoc"  :append-to-body='true' class="upload_dialog"  @close="clickEvent" width="40%">
      ...
      <div slot="footer" class="dialog-footer">
        ...
        <el-button @click.stop="uploadDocAddBtn">添加文件</el-button>  
        <el-button type="primary" @click.stop="startUploadBtn" id="startUploadBtn" :disabled="docInitIsFinish">开始上传</el-button>
      </div>
    </el-dialog>
    <!-- <el-dialog title="文件上传" :visible.sync="uploadDoc" :append-to-body='true' @close="clickEvent" width="40%" class="postil_dialog">
        <div class="postil_dialog_content">
            <div class="postil_dialog_content_tabel">
                <p class="postil_dialog_content_tabel_title">
                    <span>文件名称</span>
                    <span>大小</span>
                    <span>状态</span>
                    <span>进度条</span>
                ...
                <div class="postil_dialog_content_tabel_coutent">
                    <el-scrollbar style="height:100%">
                        <p class="postil_dialog_content_tabel_coutent_item" v-for="(item,idx) in tableData" :key="idx">
                            <div class="file_content_name">
                                <i class="file_content_name_icon"></i>
                                <span class="file_content_name_name">{{item.name}}</span>
                            </div>
        ...
        <div slot="footer" class="dialog-footer">
            <el-button type="primary" @click="uploadDocAddBtn">添加文件</el-button>
            <el-button type="primary" @click="startUploadBtn" id="startUploadBtn">开始上传</el-button>
        </div>
    </el-dialog> -->
  </div>
</template>
  ...
  methods: {
    ...
    onloadInit() {
      ...
      let param = {
        ...
        docSource: this.uploadParamData.docSource,
        projId: this.uploadParamData.projId,
        parentId: this.uploadParamData.parentId,
        auditProjectId: this.uploadParamData.auditProjectId,
        token: this.token,
        userId: this.userId,
        ...
        fileAdd: file => {
          ...
          if (that.accpet) { // 如果有格式限制，需验证文件格式
            if (this.canUpload(file.name)) { // 文件格式验证通过，加入上传队列
              that.tableData.push({
                name: file.name,
                ...
                id: file.id,
                fileIcon: this.iconFilter(file)
                // fileIcon: this.iconFilter(renameSuffix)
              });
            } else { // 文件格式不对
              var str = "允许上传的文件类型为：";
        ...
        FileUploaded: (file, docId, docName) => {
          ...
          that.$emit("docUpload", that.uploadData);
        },
        ...
        fileInitComplete: file => {
          ...
          for (let i = 0; i < that.tableData.length; i++) {
            ...
            if(that.tableData[i].state == '初始化中'){
              haveNoInit = 2;
            }
          }
          if(haveNoInit == 1){
            that.docInitIsFinish = false;
          }
        },
        //上传过程中不断触发
        UploadProgress: (up, file) => {
          ...
          for (let i = 0; i < that.tableData.length; i++) {
            if (that.tableData[i].id === file.id) {
              that.tableData[i].progress = file.percent;
          ...
          // that.uploadSpeed = (up.total.bytesPerSec / 1024).toFixed(2) + "KB";
        },
