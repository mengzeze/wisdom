  ...
  <div class="content approval">
    <div class="content_top" v-show="Toexamine">
      ...
      <p class="content_p">
        <el-breadcrumb separator="/" class="page-breadcrumb">
          <el-breadcrumb-item :to="{ path: '/maindeskindex' }"
            >主工作台</el-breadcrumb-item
          >
      ...
      <div class="content_box flex j-spaceBetween a-center">
        ...
        <div class="content_search  flex j-spaceBetween">
          ...
          <el-button
            style="height:40px"
            @click="sponsor"
            v-if="$utils.checkSystemPermission('sub_approve')"
            type="primary"
            >发起审批</el-button
          >
    ...
    <div v-show="Toexamine">
      ...
      <el-tabs
        ...
        type="border-card"
        v-model="firstTitle"
        @tab-click="content_xzs"
      >
        <el-tab-pane label="待我审批" name="0">
          <el-table
            :data="tableDataForApprove"
            v-loading="loading"
            min-height="530"
            style="width: 100%"
          ...
          >
            ...
            >
              <template slot-scope="scope">
                <el-button
                  ...
                  type="text"
                  class="ellipsis1"
                  style="padding:1px 0px;width:100%;line-height:22px;"
            ...
            >
              <template slot-scope="scope">
                <p
                  :title="
                    scope.row.typeFlag == -999 || scope.row.typeFlag == 1
                      ? ''
                      : scope.row.finanNameParent
                  "
                  class="ellipsis1"
                >
                  {{
                    scope.row.typeFlag == -999 || scope.row.typeFlag == 1
                      ? "无"
                      : scope.row.finanNameParent
                      ? scope.row.finanNameParent.substring(
                          scope.row.finanNameParent.lastIndexOf(">") + 1
                        )
                      : "--"
                  }}
                </p>
            ...
            >
              <template slot-scope="scope">
                <p :title="scope.row.remarks | msg" class="remarksss">
                  {{ scope.row.remarks | msg }}
                </p>
              </template>
            </el-table-column>
            <el-table-column
            ...
            >
              <template slot-scope="scope">
                <p :title="scope.row.username" class="ellipsis1">
                  {{ scope.row.username }}
                </p>
              </template>
            </el-table-column>
            ...
            >
              <template slot-scope="scope">
                <p :title="scope.row.approverName" class="ellipsis1">
            ...
            >
              <template slot-scope="scope">
                <span
                  v-if="scope.row.approvalStatus == '审批中'"
                  style="color:#FFC53D"
                  class="recalls_state"
                  :title="scope.row.approvalStatus"
                  >{{ scope.row.approvalStatus }}</span
                >
                <span
                  v-else-if="scope.row.approvalStatus == '已通过'"
                  style="color:#52C41A"
                  class="recalls_state"
                  :title="scope.row.approvalStatus"
                  >{{ scope.row.approvalStatus }}</span
                >
                <span
                  v-else-if="scope.row.approvalStatus == '未通过'"
                  style="color:#999999"
                  class="recalls_state"
                  :title="scope.row.approvalStatus"
                  >{{ scope.row.approvalStatus }}</span
                >
                <span
                  v-else-if="scope.row.approvalStatus == '已撤销'"
                  style="color:#999999"
                  class="recalls_state"
                  :title="scope.row.approvalStatus"
                  >{{ scope.row.approvalStatus }}</span
                >
                <span
                  v-else-if="scope.row.approvalStatus == '暂缓表决'"
                  style="color:#FA541C"
                  class="recalls_state"
                  :title="scope.row.approvalStatus"
                  >{{ scope.row.approvalStatus }}</span
                >
                <span
                  v-else-if="scope.row.approvalStatus == '有条件通过'"
                  style="color:#40A9FF"
                  class="recalls_state"
                  :title="scope.row.approvalStatus"
                  >{{ scope.row.approvalStatus }}</span
            ...
            <el-table-column width="300" align="center" label="操作">
              <template slot-scope="scope">
                <div class="oper_box">
                  <span
                    v-show="approval2"
                    @click="recalls(scope.row)"
                    v-if="scope.row.status == 1 && scope.row.isCancel == 1"
                    class="icon-operate-btn iconfont daohangchexiao"
                    title="撤销"
                  ></span>
                  ...
                  ></span>
                  <span
                    @click="guanlianshenpi(scope.row)"
                  ...
                  <span
                    @click="quxiaoguanlianshenpi(scope.row)"
                    v-if="scope.row.relatedApprove"
                  ...
                  <span
                    v-if="recallss1s && scope.row.status == 1"
                    @click="reminder(scope.row)"
                    class="icon-operate-btn iconfont zulincuijiaotixing"
                    title="催办"
                  ></span>

                  <span
                    v-if="
                      $utils.m('project_meeting') &&
                        scope.row.relatedMeeting == true
                    "
                    @click="glhy(scope.row)"
                  ...
                  <span
                    v-if="
                      $utils.m('project_vote') && scope.row.relatedVote == true
                    "
                    @click="relatedvoting(scope.row)"
                  ...
                  <span
                    v-if="
                      $utils.m('project_vote') && scope.row.relatedVote == true
                    "
                    @click="relatedQp(scope.row)"
                    class="icon-operate-btn iconfont quxiaoguanlian"
                    title="取消关联"
                  ></span>
                  <span
                    @click="Filepages(scope.row)"
                    v-if="
                      scope.row.categoryName == '文件审批' ||
                        scope.row.categoryName == '文件修订审批' ||
                        scope.row.categoryName == '目录审批' ||
                        scope.row.categoryName == '底稿目录修订审批'
                    "
        ...
        <el-tab-pane label="我已审批" name="1">
          <el-tabs
            ...
            :class="firststate ? 'has-sub-tab' : ''"
          >
            <el-tab-pane :label="'全部（' + dataTotal + '）'" name="first">
              <el-table
                :data="tableDataApprovedTotal"
                v-loading="loading"
                min-height="530"
                style="width: 100%"
                :header-cell-style="{ background: '#FAFAFA', color: '#000' }"
              >
                ...
                >
                  <template slot-scope="scope">
                    <p :title="scope.row.categoryName" class="ellipsis1">
                      {{ scope.row.categoryName }}
                    </p>
                  </template>
                </el-table-column>
                  ...
                  <template slot-scope="scope">
                    <div class="oper_box">
                      ...
                      <span
                        v-if="approval1"
                        @click="handleEdit(scope.$index, scope.row)"
                        class="oper_shenhe icon-operate-btn iconfont shenpi"
                        title="审批"
                      ></span>
                      ...
                      <span
                        ...
                        v-if="
                          scope.row.status == 0 ||
                            ...
                            scope.row.status == 1
                        "
                        class="oper_shenhejilu icon-operate-btn iconfont jilu"
                        title="审批记录"
                      ></span>
                      ...
                      <span
                        v-if="
                          $utils.m('project_vote') &&
                            scope.row.relatedVote == true
                        "
                        @click="relatedvoting(scope.row)"
                        class="icon-operate-btn iconfont guanliantoupiao"
                        title="关联投票"
              ...
              <div class="pages">
                <el-pagination
                  ...
                  @size-change="handleSizeChange(arguments, 'approvedTotal')"
                ></el-pagination>
              </div>
            </el-tab-pane>
            <el-tab-pane :label="'审批完成（' + finish + '）'" name="second">
              <el-table
                :data="tableDataApprovedFinish"
                v-loading="loading"
                min-height="530"
                style="width: 100%"
                :header-cell-style="{ background: '#FAFAFA', color: '#000' }"
              ...
              <div class="pages">
                <el-pagination
                  ...
                  @size-change="handleSizeChange(arguments, 'approvedFinish')"
                ></el-pagination>
              </div>
            </el-tab-pane>
            <el-tab-pane :label="'审批中（' + checking + '）'" name="third">
              <el-table
                :data="tableDataApprovedChecking"
                v-loading="loading"
                min-height="530"
                style="width: 100%"
                :header-cell-style="{ background: '#FAFAFA', color: '#000' }"
              ...
              <div class="pages">
                <el-pagination
                  ...
                  @size-change="handleSizeChange(arguments, 'approvedChecking')"
                ></el-pagination>
              </div>
            </el-tab-pane>
          </el-tabs>
        ...
        <el-tab-pane label="我发起的" class="wfqdinnfo" name="2">
          <el-tabs v-model="activeNameSponsor" @tab-click="handleClick">
            <el-tab-pane
              :label="'全部（' + dataTotalSponsor + '）'"
              name="first"
            >
              <el-table
                :data="tableDataMySponsorTotal"
                v-loading="loading"
                min-height="530"
                style="width: 100%"
                :header-cell-style="{ background: '#FAFAFA', color: '#000' }"
              ...
              <div class="pages">
                <el-pagination
                  ...
                  @size-change="handleSizeChange(arguments, 'sponsorTotal')"
                ></el-pagination>
              </div>
            </el-tab-pane>
            <el-tab-pane
              :label="'审批完成（' + finishSponsor + '）'"
              name="second"
            >
              <el-table
                :data="tableDataMySponsorFinish"
                v-loading="loading"
                min-height="530"
                style="width: 100%"
                :header-cell-style="{ background: '#FAFAFA', color: '#000' }"
              ...
              <div class="pages">
                <el-pagination
                  ...
                  @size-change="handleSizeChange(arguments, 'sponsorFinish')"
                ></el-pagination>
              </div>
            </el-tab-pane>
            <el-tab-pane
              :label="'审批中（' + checkingSponsor + '）'"
              name="third"
            >
              <el-table
                :data="tableDataMySponsorChecking"
                v-loading="loading"
                min-height="530"
                style="width: 100%"
                :header-cell-style="{ background: '#FAFAFA', color: '#000' }"
              ...
              <div class="pages">
                <el-pagination
                  ...
                  @size-change="handleSizeChange(arguments, 'sponsorChecking')"
                ></el-pagination>
              </div>
            </el-tab-pane>
            <el-tab-pane
              :label="'已撤销（' + cancelSponsor + '）'"
              name="fourth"
            >
              <el-table
                :data="tableDataMySponsorCancel"
                v-loading="loading"
                min-height="530"
                style="width: 100%"
                :header-cell-style="{ background: '#FAFAFA', color: '#000' }"
              ...
              <div class="pages">
                <el-pagination
                  ...
                  @size-change="handleSizeChange(arguments, 'sponsorCancel')"
                ></el-pagination>
              </div>
            </el-tab-pane>
          </el-tabs>
        ...
        <el-tab-pane label="抄送我的" name="3">
          ...
          <div class="pages">
            <el-pagination
              ...
              @size-change="handleSizeChange(arguments, 'copied')"
            ></el-pagination>
          </div>
        </el-tab-pane>
      </el-tabs>
    ...
    >
      <div class="project" style="height: calc(100vh - 300px);">
        <el-scrollbar style="height:100%">
          ...
          <div class="infowan">
            ...
            >
              业务类型:{{ currentgeneration }}
            </p>
            <p
              class="infowan-type"
              style="margin-top: 10px;margin-right: 0"
              :title="Typesoffinancing"
            >
              当前阶段:{{ Typesoffinancing }}
            </p>
            <p
              class="infowan-type"
          ...
          <div class="project1">
            <div
              style="line-height: 50px;width: 300px;"
              class="title_lable"
              v-if="prometitle"
            >
              项目信息
            ...
            <ul class="porject_name" v-if="auditormessage">
              ...
              <li
                ...
                v-for="(item, index) in typeText.form"
                :key="index"
                v-if="otherxiangm"
              >
                <i>{{ item.name }}：</i>
                <span
                  v-if="item.type.mimeType == 'text/plain'"
                  style="width: 185px;"
                >
                  <el-input
                    disabled
            ...
            <div v-if="newmesageproject">
              ...
              <ul
                ...
                class="projectnewmessage clearFix"
              >
              ...
              <ul class="projectnewmessage clearFix">
                ...
                <li class="clearFix">
                  <div class="messcontent">
                    ...
                    <span :title="proMsg.abbreviation">{{
                      proMsg.abbreviation
                    }}</span>
                  </div>
                  <div class="messcontent">
                    <label for="">业务类型：</label>
                    <span :title="proMsg.financingName">{{
                      proMsg.financingName
                    }}</span>
                  </div>
                </li>
                <li class="clearFix">
              ...
              <ul
                class="projectnewmessage clearFix"
                v-if="this.$utils.m('customer_manage')"
              >
                <div class="companymessage">中介机构</div>
                <li class="clearFix">
                  <div
                    class="messcontent clearFix"
                    v-for="(item, key, index) in messageObj"
                  >
                    <label for="">{{ key }}：</label>
              ...
              <ul class="projectnewmessage clearFix">
                ...
                <li class="clearFix">
                  <div
                    class="messcontent"
                    v-for="(value, key) in projectMemberInfo"
                  >
                    <label
                      for=""
                      :title="value.roleName"
                      class="messcontent_team_label"
                      ><span class="messcontent_team_span">{{
                        value.roleName
                      }}</span
                      >：</label
                    >
                  ...
                  <span>
                    ...
                    >
                      <el-scrollbar style="height:100%">
                        <li
                          v-for="(item,
                          index) in borrowFileLjieyue.borrowFileList"
                        >
                          <div
                            class="filename"
                            style="display: inline-flex; text-align:left;"
                          >
                            <p style="width:530px!important;cursor:auto;">
                              {{ item.fileName }}
                            </p>
                            <!-- <el-button
          ...
          <div class="wait_shfile" style="margin-left:-9px;" v-if="states">
            <div class="project1_titlebox">
              <div class="project1_titlebox_ittle" style="margin-left:12px;">
                {{ wjsh }}
              </div>
              <p
                @click="filejil"
                class="project2span color-primary"
            ...
            <ul
              id="project2"
              class="wait_shfile"
              style="margin-left:15px;height:115px;width: 97%;"
            >
              <el-scrollbar style="height:100%">
                <li
                  v-for="(item, index) in typeText.attachment"
                  v-if="item.delFlag == 0"
                  :key="item.fileId"
                  style="height:20px;"
                >
                  ...
                  <div class="fileoper" v-else>
                    <p ref="value1">
                      <input
                        type="radio"
                        @change="value1(item, index, $event)"
                        value="2"
                        :name="item.id"
                      />
                      不同意
                  ...
                  <div class="filestatus" style="margin-right: 35px;">
                    ...
                    <p
                      v-if="item.approvalStatus == '已通过'"
                      style="color: #52C41A"
                    >
                      待审批
                    </p>
                    <p
                      v-if="item.approvalStatus == '已驳回'"
                      style="color: #FF4F3E"
                    >
                      {{ item.approvalStatus }}
                    </p>
                ...
                <li
                  ...
                  :key="index"
                >
                  <div class="filename" style="text-decoration:line-through;">
                    {{ item.fileName }}
                  </div>
                </li>
              </el-scrollbar>
          ...
          <div class="project1" id="auditorfujian" v-if="audfujian">
            ...
            <div class="approval-check">
              <div class="clearFix">
                <el-checkbox
                  v-if="typeText.annex && typeText.annex.length"
                ...
                <button
                  v-if="typeText.annex && typeText.annex.length"
                  @click="downLoadFilesaaa"
                  class="fr down-load-btn"
                >
                  下载
                </button>
              </div>
              <el-checkbox-group
                class="approval-group"
                v-model="annex"
              ...
              >
                <ul>
                  ...
                  <el-checkbox
                    v-for="item in typeText.annex"
                    class="attachemnt-list-item select-box"
                    :key="item.fileId"
                    :label="item.fileId"
                  >
                    <span
                      ...
                      @click="
                        $utils.handleUploadFilePreview(item, {
                          projectId: projectId,
                          sourceName: approvalRecordObj.sourceName,
          ...
          <div class="project3 sphj_box">
            <div class="project1_titlebox">
              <div class="project1_titlebox_ittle" style="width:100px;">
                审批环节
              </div>
              <p
                @click="cares = true"
                v-if="isCareOf == 1"
                class="project2span color-primary"
                style="margin-right: -540px;margin-top: 25px;"
              >
                <el-button type="primary" size="mini">转交</el-button>
              </p>
            ...
            <ul>
              <li
                style="margin-top:20px;margin-left:3px;text-align:left;width: 86%;"
              >
                <i style="color:red">*</i>
                ...
                <i
                  v-for="(item, index) in radioId"
                  :key="index"
                  style="padding-right: 15px;"
                >
                  <input
                    ref="radios"
                    @change="record(item, index)"
                    type="radio"
                    name="name"
                  />
                  <i>{{ item.name }}</i>
          ...
          <div
            class="proend"
            style="margin-top:18px;max-height: 126px;overflow-y: auto;height:auto;"
          >
            <p style="padding-bottom: 7px;margin-top:10px;">抄送人:</p>
            <span v-if="deployObjputong.length > 0">&nbsp;&nbsp;</span>
            <el-button
              type="primary"
              @click="optPrinFnputong(1)"
              size="small"
              style="margin-top:-1px;"
              >选择人员</el-button
            >
            <el-tag
              v-for="(item, index) in colyObjputong"
              :key="'info2' + index"
              class="userbtn"
            ...
            <el-tag
              v-for="(item, index) in deployObjputong"
              :key="index"
              class="userbtn"
              style="margin-left:10px;margin-bottom:10px;"
              @close="handle_Closeputong(index, 0)"
              :closable="!chack_buleputong"
            >
              {{ item.name }}
            </el-tag>
          </div>
          <div class="fj_box" style="padding-left:16px;text-align:left;">
            ...
            <add-attachment
              ref="attachment"
              :projectId="projectId"
              @clearApprovalType="clearApprovalType"
              :currentApprovalType="currentApprovalType"
              :onlyLocal="newmesageproject"
              @select="$utils.handleSelect(arguments, selectedFileList)"
            ></add-attachment>
            <ul
              style="text-align: left;margin-left: 48px;"
                ...
                <p class="attachemnt-list-doc" style="float: none;">
                  ...
                  <span>
                    ...
                    <el-button
                      type="text"
                      @click="
                        $utils.handleDeleteSelected(selectedFileList, item)
                      "
                      >删除</el-button
                    >
                  </span>
            ...
            <ul v-if="1 === 2" id="project2" style="margin-left:17px;">
              <el-scrollbar style="height:100%">
                <li
                  v-for="(item, index) in shagreatlist"
                  :key="index"
                ...
                >
                  ...
                  <span
                    type="text"
                    @click="yulan3(item)"
                    style="width:518px; overflow: hidden;text-overflow: ellipsis;white-space: nowrap;color: #0061a9;cursor: pointer;"
                    >{{ item.name }}</span
                  >
                  <i
                    class="project2span color-primary"
                ...
                <li
                  v-for="(item, index) in shagchaunlist"
                  :key="index"
                  class="upload_file_list"
                >
                  <!-- <span class="hide_text">{{item.name}}</span> -->
      ...
      <div id="dialog-footer">
        <el-button size="medium" style="margin-right:12px;" @click="closes"
          >取 消</el-button
        >
        <span id="sub_approve">
          <el-button
          ...
          >
            <el-form-item label="提醒方式：" class="remindRight">
              <el-checkbox-group v-model="remindWay" :min="1">
                <el-checkbox
                  v-for="way in remindWayGrounp"
                  :label="way.label"
                  :key="way.value"
                  >{{ way.label }}</el-checkbox
                >
              </el-checkbox-group>
            </el-form-item>
            <el-form-item label="接收人员：" class="remindRight">
          ...
          <div id="records">
            <el-scrollbar style="height: 100%">
              <ul style="text-align:left;">
                ...
                <li>
                  <p
                    style="font-weight:400;text-align:left;float: left;margin-left: 0px;font-weight: bold;color: black;font-size: 14px;"
                  >
                    申请内容：
                  </p>
                  <!-- margin-top: -25px;margin-bottom: 10px; -->
                  <div
                    style="float: left;width: 468px; height:72px; margin-left: 26px;"
                  >
                ...
                <li>
                  <p
                    style="font-weight:400;text-align:left;float: left;margin-left: 0px;font-weight: bold;color: black;font-size: 14px;margin-top:10px;"
                  >
                    抄送人：
                  </p>
                  <div
                    style="float: left;width: 468px; height:120px; margin-left: 26px;margin-top:8px;max-height: 150px;overflow-y: auto;"
                  >
                    <el-tag
                      v-for="(item, index) in colyObj"
                      :key="'info2' + index"
                      class="userbtn"
                      style="margin-left:10px;margin-bottom:10px;"
                    >
                ...
                <div v-if="attachmentShow" class="">
                  ...
                  >
                    <el-checkbox
                      :indeterminate="isIndeterminatefujian"
                      v-model="checkAllfujian"
                      @change="handleCheckAllChangefujian"
                      class="fl"
                      style="font-weight: bold;color: black;"
                      >上传附件:</el-checkbox
                    >
                    <button
                      @click="downLoadFilesfujian"
                      class="fr down-load-btn down-load-btn-right"
                    >
                      下载
                    </button>
                  </div>
                  ...
                  >
                    <el-scrollbar>
                      <div
                        class="files-box"
                        v-for="item in approved_memo.attachment"
                        :key="item.fileId"
                        :label="item.fileId"
                      >
                        ...
                        <el-checkbox-group
                          class="approval-group"
                          v-model="annexfujian"
                          @change="handleCheckedCitiesChangefujian"
                        >
                          <ul>
                            <el-checkbox
                              class="attachemnt-list-item select-box"
                ...
                <div
                  class="sh_list"
                  v-for="(item, i) in approved_memo.list"
                  :key="i"
                >
                  <li>
                  ...
                  <li>
                    ...
                    <span
                      v-if="item.ownerName == null || item.ownerName == ''"
                      class="lispsan"
                      style="margin-left: 6%;"
                      >{{ item.username }}</span
                    >
                    <span v-else class="lispsan" style="margin-left: 6%;"
                      >{{ item.ownerName }}
                      <i class="iconfont weibiaoti40" aria-hidden="true"></i>
                      {{ item.username }}</span
                    >
                  </li>
                  <li>
                  ...
                  <li v-if="item.comment != '' && item.comment != null">
                    ...
                    <span :title="item.comment" class="comments">{{
                      item.comment
                    }}</span>
                  </li>
                  <li v-if="item.ownerContext != null">
                    ...
                    <span
                      :title="item.ownerContext"
                      style="margin-left: 4%;display: inline-block;max-height: 116px;overflow: auto;"
                      >{{ item.ownerContext }}</span
                    >
                    <!--
            <el-input style="width: 85%;" type="textarea" :rows="2" maxlength="1000" placeholder="请输入转交理由" v-model="carestextarea"> -->
                  ...
                  <div class="dialogs_files" v-if="item.commentAttachmentshow">
                    <div class="clearFix">
                      <el-checkbox
                        :indeterminate="item['isIndeterminatefujsecond']"
                        v-model="item['checkAllfujsecond']"
                        @change="
                          checked =>
                            handleCheckAllChangefujsecond(
                              checked,
                              i,
                              item,
                              approved_memo.list
                      ...
                      <button
                        @click="downLoadFilesfujsecond(i, item)"
                        class="fr"
                        style="margin-right:30px;background:none;"
                      >
                        下载
                      </button>
                    </div>
                    ...
                    <div class="" style="">
                      <li
                        v-for="(o, ind) in item.commentAttachment"
                        :key="ind"
                        class="attach-files"
                      >
                        <el-checkbox-group
                          ...
                          @change="
                            checked =>
                              handleCheckedCitiesChangefujsecond(
                                checked,
                                i,
                                item,
                                o
          ...
          <div id="records">
            <ul>
              ...
              <div v-for="(item, i) in review.list" :key="i">
                ...
                <li>
                  <span class="linspans">审批结果：</span>
                  <span class="result" v-if="item.status != '同意'">{{
                    item.status
                  }}</span>
                  <span
                    class="result"
                ...
                <div class="dialogs_files">
                  ...
                  <div class="dialogs_files_box">
                    <el-scrollbar style="height:100%">
                      <li
                        v-for="(ite, index) in item.recordAttachment"
                        :key="index"
                        style="height:20px;"
                      >
                        <div v-if="ite.state == 0">
                          ...
                          <div v-else class="gdt">
                            ...
                            >
                            <i
                              style="margin-right: 5px;float: right;font-size: 9px;color: #999999;"
                              >待审批</i
                            >
                          </div>
                        </div>
                        <div v-if="ite.state == 1">
                          ...
                          <div v-else class="gdt">
                            ...
                            >
                            <i
                              style="margin-right: 5px;float: right;color: #999999;"
                              >通过</i
                            >
                          </div>
                        </div>
    ...
    <el-dialog
      ...
      center
      :close-on-click-modal="false"
      :visible.sync="dialogVisiblewenj"
      width="430px"
    >
      <div>
        <div class="dialos_box">
        ...
        <el-scrollbar style="height:100%">
          <el-form label-width="130px" align="left" class="demo-ruleForm">
            ...
            <el-form-item label="新增会议：" v-if="showNewMeeting" prop="type">
              <el-button type="primary" size="medium" @click="addresMeeting"
                >新增会议</el-button
              >
            </el-form-item>
          </el-form>
          <span class="ques_button">
            <el-button size="medium" type="primary" @click="RelationMeet"
              >确定</el-button
            >
      ...
      <div>
        <el-scrollbar style="height:100%">
          ...
          >
            <ul align="left" class="riwjn_we" style="margin-left:24px;">
              <li class="usernanmestlt" style="font-size:14px;">
                <i
                  style="color:red;padding-right: 5px;    margin-left:4px;text-align:left;"
                  >*</i
                >参会人员：
              </li>
              <li class="usernanmes" style="font-size:14px;">
                ...
                >
                  ...
                  <button
                    ...
                    @click="deletusid(1, index)"
                  ></button>
                </span>
                <span>&nbsp;&nbsp;</span>
                <el-button type="primary" @click="optPrinFn(1)" size="mini"
                  >选择人员</el-button
                >
              </li>
              <!-- <li>参会人员:</li>
            ...
            <ul align="left" class="riwjn_we" style="margin-left:42px;">
              <li class="usernanmestlt" style="font-size:14px;width:76px;">
                列席人员：
              </li>
              <li class="usernanmes">
                <span
                  class="userbtn"
                  style="padding-right:10px;"
                  v-for="(item, index) in meeting.copyPerson"
                  :key="index"
                >
                  {{ item.name }}
                  <button
                    class="deletro delusebtn"
                    @click="deletusid(2, index)"
                  ></button>
                </span>
                <span>&nbsp;&nbsp;</span>
                <el-button type="primary" @click="optPrinFn(2)" size="mini"
                  >选择人员</el-button
                >
              </li>
            </ul>
            ...
            <el-form-item label="备注：" style="margin-top:10px;">
              <el-input
                type="textarea"
                :rows="2"
                v-model.trim="meeting.pro_remark"
                placeholder="请输入内容"
                :maxlength="2000"
            ...
            <ul align="left" class="riwjn_we">
              ...
              <li class="usernanmes">
                <span
                  class="userbtn"
                  style="padding-right:10px;"
                  v-for="(item, index) in meeting.copychaoon"
                  :key="index"
                >
                  {{ item.name }}
                  <button
                    class="deletro delusebtn"
                    @click="deletusid(5, index)"
                  ></button>
                </span>
                <span>&nbsp;&nbsp;</span>
                <el-button type="primary" @click="optPrinFn(7)" size="mini"
                  >抄送人员</el-button
                >
              </li>
            </ul>
            ...
            <el-form-item label="附件：" style="margin-top:10px;" align="left">
              <add-attachment
                ref="attachmentMeeting"
                :projectId="queryDocProjectId"
                @select="$utils.handleSelect(arguments, selectedFileMeeting)"
              ></add-attachment>

              <ul style="text-align: left;" class="attachment-wrap">
                <li
                  v-for="item in selectedFileMeeting"
                  ...
                  <p class="attachemnt-list-doc">
                    ...
                    <span>
                      ...
                      <el-button
                        type="text"
                        @click="
                          $utils.handleDeleteSelected(selectedFileMeeting, item)
                        "
                        >删除</el-button
                      >
                    </span>
          ...
          <span class="ques_button" style="float:right;margin-right:40px;">
            <el-button size="medium" type="primary" @click="aDDupMeet"
              >确定</el-button
            >
        ...
        <el-scrollbar style="height:100%">
          <el-form ref="ruleForm4" label-width="140px" class="demo-ruleForm">
            <el-form-item label="选择投票：" align="left" prop="region">
              <!-- {{CvoteList_id}} -->
              <el-select
                v-model="CvoteList_id"
                filterable
                placeholder="请选择投票"
              >
                <!-- CvoteList -->
                <el-option
                  v-for="item in CvoteList"
                  :key="item.id"
                  :label="item.name"
                  :value="item.id"
                ></el-option>
            ...
            <el-form-item
              ...
              prop="type"
            >
              <el-button type="primary" size="medium" @click="addCvote"
                >新增投票</el-button
              >
            </el-form-item>
          </el-form>
          <span class="ques_button">
            <el-button size="medium" type="primary" @click="Relatedvoting"
              >确定</el-button
            >
      ...
      <div style="height:470px;">
        <el-scrollbar style="height:90%">
          ...
          <ul align="left" class="riwjn_we" style="margin-left:35px;">
            ...
            <li>
              <span
                v-for="(item, index) in msgObj.Voterusids"
                :key="index"
            ...
            <li style="padding-left:5px;margin-top:5px;">
              <el-button type="primary" size="mini" @click="optPrinFn(3)"
                >选择人员</el-button
              >
            </li>
          </ul>
          ...
          <p style="color:#999999" v-if="jincarr.length > 0">
            该投票需要角色:
            <span v-for="(item, index) in jincarr" :key="index"
              >{{ item.roleName }},不少于{{ item.count }};</span
            >
          </p>
          <ul align="left" class="riwjn_we">
            <li>抄送人员：</li>
            <li>
              <span
                v-for="(item, index) in msgObj.copyPerson"
                :key="index"
                style="margin-left:15px;margin-top:5px"
                size="mini"
                plain
            ...
            <li style="padding-left:5px;margin-top:5px;">
              <el-button type="primary" size="mini" @click="optPrinFn(4)"
                >选择抄送人员</el-button
              >
            </li>
          </ul>
          <el-form label-width="120px" align="left">
            <el-form-item label="附件：" style="margin-top:10px;" align="left">
              <add-attachment
                ...
                :projectId="queryDocProjectId"
                @select="$utils.handleSelect(arguments, selectedFileVote)"
              ></add-attachment>
              ...
              <ul style="text-align: left;height:60px;">
                ...
                <el-scrollbar style="height:100%">
                  <li
                    v-for="item in selectedFileVote"
                    class="attachemnt-list-item"
                  >
                    <p class="attachemnt-list-doc">
                      <span
                        ...
                        @click="
                          $utils.handleUploadFilePreview(item, {
                            projectId: queryDocProjectId,
                            sourceName: '主工作台-我的审批-新增投票',
                            projectName: msgObj.pro_name
                          })
                        "
                        :title="item.docName"
                        >{{ item.docName }}</span
                      ...
                      <span>
                        <el-button
                          type="text"
                          @click="
                            $utils.handleDeleteSelected(selectedFileVote, item)
                          "
                          >删除</el-button
                        >
                      </span>
        ...
        <span style="float:right;margin-right:40px;">
          <el-button size="medium" type="primary" @click="AddVote"
            >确定</el-button
          >
    ...
    >
      <div>
        <ul class="caresli">
          ...
          <li>
            <span>
              <i style="color:red;padding-right: 5px">*</i>转交人员：</span
            >
            <span @click="selectPeople"
              ><el-button
          ...
          <li>
            <p style=" float: left">
              <i style="color:red;padding-right: 5px">*</i>转交理由：
            </p>
            <el-input
              style="width: 85%;"
              type="textarea"
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button size="medium" type="primary" @click="caresqd"
          >确 定</el-button
        >
      </span>
    </el-dialog>
    ...
    <findall-deptuserputong
      :findFlagShowputong.sync="findFlagputong"
      v-on:findAllUserputong="findAllUserputong"
      :findUserObjputong="
        user_numputong === 1 ? findUserObjputong : findUserObjMputong
      "
      :findStateputong="{ state: 0 }"
      :checkStateputong="{ state: 2 }"
    ></findall-deptuserputong>
    <related-approval
      @queryData="pending"
      :examObj="examObj"
      v-if="relatedExamVisible"
      :relatedExamVisible.sync="relatedExamVisible"
    ></related-approval>
    <cancel-related-approval
      @queryData="pending"
      :examObj="examObj"
      :queryList="queryList"
      :noData="noData"
      v-if="cancelRelatedExamVisible"
      :cancelRelatedExamVisible.sync="cancelRelatedExamVisible"
    ></cancel-related-approval>
  </div>
</template>
<script>
// 新建日程组件
import calendar from "@/pages/front/maindesk/calendar/newcalendar";
// 选择人员
...
//
import setTypeso from "@/pages/front/Lotus/setTypeso";
...
export default {
  ...
  data() {
    return {
      ...
      noData: false, //是否有关联
      currentApprovalType: "", // 判断审批弹窗附件上传方式 7为新建项目审批类型 与未关联项目一致 不可使用底稿和项目文档
      arraydatas: [] // 单多选下标集合
    };
  },
  created() {
    this.Toexamine =
      this.$route.path == "/backlogmatter" || this.$route.path == "/Filepage"
        ? false
        : true;
  ...
  methods: {
    ...
    quxiaoguanlianshenpi(obj) {
      ...
      var sendData = {
        ...
        userId: this.userId,
        data: {
          approvalId: obj.id, //当前审批id
          taskDefKey: obj.taskDefKey //审批节点id
        }
      };
      this.$post("/info/audit/relevancy/getApproval", sendData)
        .then(res => {
    ...
    findAllUserputong(data) {
      if (!data || !data.length) {
        ...
        this.user_numputong == 1 && (this.deployObjputong = []);
      }
      //  返回的数据分为默认的和其他的选择的，
      (this.colyObjputong = []), (this.deployObjputong = []);
      this.user_numputong == 1 &&
        data.forEach(v => {
          if (v.originData || v.defaultUser == 1) {
            this.colyObjputong.push(v);
          } else {
            this.deployObjputong.push(v);
          }
        });
    ...
    optPrinFnputong(num) {
      ...
      if (num == 1) {
        if (!this.colyObjputong || !this.colyObjputong.length) {
          this.findUserObjputong =
            this.$utils.cloneDeepArray(this.deployObjputong) || [];
          return;
        }
        this.findUserObjputong = [
          ...this.colyObjputong,
    ...
    handleCheckedCitiesChange(value) {
      ...
      this.isIndeterminate =
        checkedCount > 0 && checkedCount < this.typeText.annex.length;
    },
    handleCheckAllChangefujian(val) {
      this.annexfujian = val
        ? this.approved_memo.attachment.map(v => v.fileId)
        : [];
    ...
    handleCheckedCitiesChangefujian(value) {
      ...
      let checkedCount = value.length;
      this.checkAllfujian =
        checkedCount === this.approved_memo.attachment.length;
      this.isIndeterminatefujian =
        checkedCount > 0 && checkedCount < this.approved_memo.attachment.length;
    },
    handleCheckAllChangefujsecond(val, i, item, list) {
      // 流程种审批人附件全全全全全选
      // console.log(this.approvelistArray)
      // console.log(i)
      // console.log(item)
      // this.arraydatas = list[i].commentAttachment;
      // console.log(list[i])
      item.checkAllfujsecond = val;
      item.isIndeterminatefujsecond = false;
      let aaa = item.commentAttachment.slice();
      item.attachments = val ? aaa.map(v => v.fileId) : [];
      // console.log(val)
      // console.log(item.attachments)
      this.arraydatas = val ? item.commentAttachment.map(v => v) : [];
      console.log("this.arraydatas", this.arraydatas);
    },
    handleCheckedCitiesChangefujsecond(val, i, item, o) {
      // 流程种审批人附件单单单单单选
      // val 选中id集合
      // i 当前选中下标
      // item 当前节点所有审批人信息
      // o 当前选中附件
      let checked = val.indexOf(o.fileId) > -1;
      checked
        ? this.arraydatas.findIndex(v => v.fileId === o.fileId) < 0 &&
          this.arraydatas.push(o)
        : (this.arraydatas = this.arraydatas.filter(v => v.id !== o.id));
      item.attachments = val;
      // 处理全选和
      let checkedCount = val.length;
      let totalLength = item.commentAttachment.length;
      item.checkAllfujsecond = checkedCount === totalLength;
      item.isIndeterminatefujsecond =
        checkedCount > 0 && checkedCount < totalLength;
      console.log("this.arraydatas", this.arraydatas);
    },
    // 获取提醒方式
    getPromptType() {
      this.$post("/sys/getNoticeWayConfig")
        .then(res => {
          if (this.code.codeNum.SUCCESS == res.code) {
            res.data.email == 1 &&
              this.remindWayGrounp.push({ label: "邮件", value: 1 });
            res.data.sms == 1 &&
              this.remindWayGrounp.push({ label: "短信", value: 2 });
    ...
    caresqd() {
      ...
      var arr = [];
      this.caretags.forEach((ele, index) => {
        arr.push(ele.userId);
      });
      var dataObj = {
        token: this.token,
      ...
      this.$post("/info/audit/handOver", dataObj)
        .then(response => {
          if (response.code == 0) {
            ...
            _this.pending(
              "",
              _this.onPageChangeinfo,
              _this.handleSizeChangeinfo
            );
            this.$emit("updatas", true);
    ...
    glhy(row) {
      ...
      this.$utils.getProjectPermPromise(row.projectId).then(res => {
        this.showNewMeeting =
          res && res.findIndex(v => v === "project_add_mett") > -1;
      });
    ...
    aDDupMeet() {
      ...
      if (this.meeting.pro_mode.length === 0 && this.meeting.remindType > 0) {
    ...
    //取消会议
    cancelMeet() {
      this.$refs["attachmentMeeting"] &&
        this.$refs["attachmentMeeting"].close();
      this.meeting.name = "";
      this.meeting.typeName = "";
      this.meeting.pro_time = "";
      this.meeting.list_time = "";
      ...
      this.$post("/info/audit/get_vote_list", dataObj)
        .then(response => {
          if (response.code == that.code.codeNum.SUCCESS) {
            ...
            this.$utils.getProjectPermPromise(row.projectId).then(res => {
              this.showNewVote =
                res && res.findIndex(v => v === "project_add_vote") > -1;
    ...
    AddVote() {
      ...
      if (this.msgObj.pro_mode.length === 0 && this.msgObj.remindType > 0) {
        this.$message.error("请选择提醒方式");
        return;
      }
      if (this.msgObj.Voterusids !== "") {
    ...
    yulan5(item) {
      ...
      var previewData = {
        ...
        docName: item.fileName,
        projectName: this.projectName,
        sourceName: this.itemState[1].source
      };
      this.$store.commit("previewAllFn", previewData);
    },
    // //  新增中介机构
    ...
    findAllUser(data) {
      ...
      if (this.zhongnum == 1) {
        ...
        if (this.meeting.copyPerson !== "") {
          this.meeting.copyPerson = this.$utils.uniqBy(
            this.meeting.copyPerson,
            "id"
          );
        }
        ...
        if (this.msgObj.Voterusids.length) {
          this.msgObj.Voterusids = this.$utils.uniqBy(
            this.msgObj.Voterusids,
            "id"
          );
        }
        ...
        if (this.msgObj.copyPerson !== "") {
          this.msgObj.copyPerson = this.$utils.uniqBy(
            this.msgObj.copyPerson,
            "id"
          );
        }
        ...
        if (this.meeting.copychaoon !== "") {
          this.meeting.copychaoon = this.$utils.uniqBy(
            this.meeting.copychaoon,
            "id"
          );
        }
      }
      ...
      return await this.$post(url, data)
        .then(response => {
          ...
          for (var i = 0; i < _this.messagefailed.length; i++) {
            ...
            if (
              !_this.messagefailed[i].email &&
              checkedRemindNames.indexOf("邮件") != -1
            ) {
              _this.errorMsg += name + "的邮箱发送失败；";
            }
            if (
              !_this.messagefailed[i].mobile &&
              checkedRemindNames.indexOf("短信") != -1
            ) {
          ...
          if (_this.errorMsg) {
            let errorMsg =
              _this.errorMsg + "请在个人信息中完善！其余均发送成功";
    ...
    pending(text, currentPage = 1, currentPageval = 10) {
      ...
      this.$post(url, data)
        .then(response => {
          ...
          this.firstTitle = "0";
          this.index = 0;
          this.tableDataForApprove = response.data.list;
    ...
    content_xzs(tab) {
      ...
      this.activeNameSponsor = "first";
      this.handleSizeChangeinfo = "";
      // this.pageSize = 10
      if (index == 0) {
        // alert(1)
        this.pageSizeForApprove = 10;
        this.currentPageForApprove = this.isLocation
          ? this.locationData.pageNo
          : 1;
        this.first1 = true;
        this.first2 = false;
        this.first3 = false;
        ...
        this.currentPageMySponsorTotal = this.isLocation
          ? this.locationData.pageNo
          : 1;
    ...
    handleEdit(index, row) {
      ...
      if (row.approvalType == 9) {
        ...
        var that = this;
        this.$post("/info/audit/getDocPaperBorrowingByApproveId", dataObj)
          .then(response => {
      ...
      if (row.approvalType == 8) {
        ...
        var that = this;
        this.$post("/info/audit/getSealUpStageByApproveId", dataObj)
          .then(response => {
            var data = response.data;
            console.log(data);
            if (response.code == that.code.codeNum.SUCCESS) {
      ...
      if (row.approvalType == 7) {
        ...
        var that = this;
        this.$post("/info/project/getProjectDetail", dataObj)
          .then(response => {
            var data = response.data;
            console.log(data);
            if (response.code == that.code.codeNum.SUCCESS) {
      ...
      var that = this;
      this.$post("/info/audit/check_approval", dataObj).then(function(
        response
      ) {
        if (response.code == that.code.codeNum.SUCCESS) {
    ...
    interFn(val) {
      ...
      var that = this;
      this.$post("/info/crm/companyCustomer", crmObj)
        .then(response => {
          var data = response.data;
          if (response.code == that.code.codeNum.SUCCESS) {
            //that.editData = data;
    ...
    shenhe(num) {
      ...
      this.$post(urls, datas)
        .then(response => {
          ...
          response.data.carbonCopy &&
            response.data.carbonCopy.forEach(v => {
              v.originData = true;
              v.userId = v.id;
            });
    ...
    approved(item) {
      ...
      this.$post(urls, datas)
        .then(response => {
          ...
          if (
            _this.approved_memo.attachment &&
            _this.approved_memo.attachment.length > 0
          ) {
          ...
          // _this.approved_memo.list.forEach(v => {

          // })
          //list是审批记录的层级，commentAttachment层级下的审批人的附件集合
          if (_this.approved_memo.list.length) {
            _this.approved_memo.list.forEach(val => {
              this.$set(val, "isIndeterminatefujsecond", false);
              this.$set(val, "checkAllfujsecond", false);
              this.$set(val, "attachments", []);
              // val.isIndeterminatefujsecond = false
              // val.checkAllfujsecond = false
              // val.attachments = []
    ...
    downLoadFilesfujian() {
      ...
      if (uploadData.length == 1) {
        ...
        let name = this.projectName
          ? `${this.projectName.substring(0, 10)}-`
          : "";
        var data = {
          random: Math.random(),
          docIdStr: uploadData.map(v => v.docId).join(","),
          zipName: name + "审批附件.zip"
        };
        console.log(data);
        this.$store.commit("export", {
          url: "/rfs/downloadDocsZip",
    ...
    downLoadFilesfujsecond(i, data) {
      ...
      this.arraydatas.forEach(item => {
        data.commentAttachment.forEach(it => {
          if (item.fileId == it.fileId && it.delFlag != 1) {
            if (this.$store.state.isPC) {
              uploadData.push({
                docId: it.fileId,
                docName: it.fileName
    ...
    reminder(item) {
      ...
      this.$post(urls, datas)
        .then(response => {
          ...
          let project = item.projectName ? `${item.projectName}的` : "";
          //var aa= &nbsp;

          // _this.textaress =
          //   `【智慧投行】 ${this.$store.state.loginObject.userName} ：提醒您尽快审批关于${project}${item.categoryName}，可在'我的审批-待我审批中'进行查看，该审批提交时间为${this.$moment().format("YYYY-MM-DD HH:mm:ss")}`
          ...
          if (item.projectName) {
            ...
            _this.textaress =
              "【智慧投行】 " +
              this.$store.state.loginObject.userName +
              "：" +
              "提醒您尽快审批关于" +
              item.projectName +
              "项目的" +
              item.categoryName +
              "，可在'我的审批-待我审批中'进行查看，该审批提交时间为 " +
  ...
  .projectnewmessage {
    ...
    li {
      ...
      .messcontent {
        ...
        .messcontent_team_label {
          width: 120px;
          display: flex;
          float: left;
          .messcontent_team_span {
            display: inline-block;
            width: 120px;
            overflow: hidden;
            text-overflow: ellipsis;
...
.borrowZtree li span.button.diy01_ico_open {
  ...
  background: url("../../../../assets/manuscript_icon/examopen_icon.png")
    no-repeat;
