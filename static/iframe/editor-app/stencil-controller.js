 ...
 */
'use strict';
// item.assignment.auditAssignee
// item.assignment.copyAssignee
//请copyAssignee
function parentTest(item){
  if(item == undefined){
    window.parent.parentTestes(['6'],true)
    return
  }
  console.log(item)
  let url
  if(item.assignment.assignee == undefined || typeof item.assignment.assignee == 'number'){
    url = '/getNewProcessApproNameAndCarbonName'
    var typeObj ={
        'token': JSON.parse(sessionStorage.getItem("userToken")),
        'userId': JSON.parse(sessionStorage.getItem("userId")),
        "data": {
            auditAssignee:{
              "initiator":item.assignment.auditAssignee.initiator,
              "lastStep":item.assignment.auditAssignee.lastStep,
              "usersStr":item.assignment.auditAssignee.users.join(','),
              "sysRoleStr":item.assignment.auditAssignee.sysRole.join(','),
              "projectRoleStr":item.assignment.auditAssignee.projectRole.join(','),
              "supervisor":item.assignment.auditAssignee.supervisor,
              "supervisorNum":item.assignment.auditAssignee.supervisorNum,
              "lastSupervisor":item.assignment.auditAssignee.lastSupervisor
            },
            copyAssignee:{
              "initiator":item.assignment.copyAssignee.initiator,
              "lastStep":item.assignment.copyAssignee.lastStep,
              "usersStr":item.assignment.copyAssignee.users.join(','),
              "sysRoleStr":item.assignment.copyAssignee.sysRole.join(','),
              "projectRoleStr":item.assignment.copyAssignee.projectRole.join(','),
              "supervisor":item.assignment.copyAssignee.supervisor,
              "supervisorNum":item.assignment.copyAssignee.supervisorNum,
            }
          }
       }
  }else{
    url = '/getProcessApproNameAndCarbonName'
    var typeObj ={
      'token': JSON.parse(sessionStorage.getItem("userToken")),
      'userId': JSON.parse(sessionStorage.getItem("userId")),
      "data": {
        'assignment':item.assignment.assignee
      }
    }
  }
  ...
  if(item != ""){
    ...
    oAjax.open('POST',  ACTIVITI.CONFIG.contextRoot + url, true);//false表示同步请求
    // oAjax.open('POST', 'http://192.168.6.62:19100/info/service/getProcessApproNameAndCarbonName', true);//false表示同步请求
    oAjax.setRequestHeader("Content-type", "application/json;charset=UTF-8");
    oAjax.send(JSON.stringify(typeObj));
    oAjax.onreadystatechange = function() {
      if (oAjax.readyState == 4 && oAjax.status == 200) {
        var data = JSON.parse(oAjax.responseText);
        window.parent.parentTestes(data.data);
        setTimeout(function(){
          var arr = document.getElementsByClassName("am-fade");
    ...
    var aitm = ""
    window.parent.parentTestes(aitm);
    setTimeout(function(){
      var arr = document.getElementsByClassName("am-fade");
      for(var i=0;i<arr.length;i++){
        arr[i].style.display = "none";
...
angular.module('activitiModeler')
  .controller('StencilController', ['$rootScope', '$scope', '$http', '$modal', '$timeout', function ($rootScope, $scope, $http, $modal, $timeout) {
    ...
    $scope.showProperty = function(title){
      var show = true
      if(title === '是否依赖投票结果') {
        show = $scope.hasVote
      } else if (title === '是否依赖会议截止时间') {
        show = $scope.hasMeeting
      } else if (title === '节点关联') {
        // 现在加了关联审批的选择项 展示不需要控制关联投票和会议了
        // show = ($scope.hasVote || $scope.hasMeeting)
        show =  true
      }
      return show
    };
    ...
    $scope.editorFactory.promise.then(function () {
      ...
      $http({
        ...
        for (var stencilIndex = 0; stencilIndex < data.stencils.length; stencilIndex++)
        {
          ...
          var stencilItem = {'id': data.stencils[stencilIndex].id,
            ...
            console.log(stencilItem, '+111111111111111111+++data.stencils')
        ...
        console.log($scope.quickMenuItems, $scope.morphRoles)
      }).
      ...
      $scope.editor.registerOnEvent(ORYX.CONFIG.EVENT_SELECTION_CHANGED, function (event) {
        ...
        if (shapes && shapes.length > 0) {
          ...
          console.log(stencil, properties, '22000')
          for (var i = 0; i < properties.length; i++) {
            ...
            // console.log(propertyConfig, 'propertyConfig', selectedShape.properties[key])
            ...
            if (propertyConfig === undefined || propertyConfig === null) {
              ...
              console.log(123, selectedShape.properties[key], property, property.isHidden())
              var currentProperty = {
                'key': key,
              ...
              console.log(currentProperty, 'currentProperty-------------',currentProperty.value, selectedItem.properties)
            }
          }

          // Need to wrap this in an $apply block, see http://jimhoskins.com/2012/12/17/angularjs-and-apply.html
          // console.log(selectedItem, selectedShape, 36)
    ...
    $scope.propertyClicked = function (index) {
      ...
      // 人员点击弹框
      if(index == 4){
        if($scope.selectedItem.properties[index].value != ""){
          // opt_user_value = $scope.selectedItem.properties[index].value.assignment.assignee;
          // parentTest($scope.selectedItem.properties[index].value.assignment.assignee);
          parentTest($scope.selectedItem.properties[index].value);
        }else{
          parentTest();
        }
