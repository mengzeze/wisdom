...
var KisBpmAssignmentPopupCtrl = ['$scope', '$modal', '$window', function ($scope, $modal,$window) {
  ...
  $window.personSet = $scope.save = function () {
    // 处理弹窗数据
    var actObj = JSON.parse(sessionStorage.getItem('actObj'));
    // 处理后的数据赋值
    let sysRole = [],projectRole = [] ,coypusersys = [],copyprojectRole = []
    console.log(actObj)

    function getdata(arr,sysarr,front){
      for(var n=0; n<arr.length; n++){
        if(arr[n].front == front){
          sysarr.push(arr[n].id)
        }
      }
      return sysarr
    }
    $scope.assignment.assignee = Math.random()
    $scope.property.value = {}
    $scope.property.value.assignment = {};
    $scope.property.value.assignment = $scope.assignment;
    var setuser = []
    if(actObj.setuser.consumer) {
      for(var s=0; s<actObj.setuser.consumerarr.length; s++)
      setuser.push(actObj.setuser.consumerarr[s].id)

    }
    var coypuser = []
    if(actObj.coypuser.consumer) {
      for(var c=0; c<actObj.coypuser.consumerarr.length; c++)
      coypuser.push(actObj.coypuser.consumerarr[c].id)
    }
    handleAssignmentInput($scope);
    $scope.property.value.assignment.auditAssignee = {
      "initiator":actObj.setuser.Approvaluser != undefined?true :false, 
      "lastStep":actObj.setuser.Approverlast != undefined?true:false ,
      "users":setuser,
      "sysRole":actObj.setuser.role != undefined ? getdata(actObj.setuser.rolearr,sysRole,'backstage'):[],
      "projectRole":actObj.setuser.role != undefined ? getdata(actObj.setuser.rolearr,projectRole,'front'):[],
      "supervisor":actObj.setuser.Supervisor != undefined ?true:false,
      "supervisorNum":actObj.setuser.Supervisor != undefined?actObj.setuser.Supervisorstate:'',
      "lastSupervisor":actObj.setuser.Supervisor != undefined?actObj.setuser.isManagerInstead:false
    }
    $scope.property.value.assignment.copyAssignee = {
      "initiator":actObj.coypuser.Approvaluser != undefined?true :false, 
      "lastStep":actObj.coypuser.Approverlast != undefined?true:false ,
      "users":coypuser,
      "sysRole":actObj.coypuser.role != undefined ? getdata(actObj.coypuser.rolearr,coypusersys,'backstage'):[],
      "projectRole":actObj.coypuser.role != undefined ? getdata(actObj.coypuser.rolearr,copyprojectRole,'front'):[],
      "supervisor":actObj.coypuser.Supervisor != undefined ?true:false,
      "supervisorNum":actObj.coypuser.Supervisor != undefined ?actObj.coypuser.Supervisorstate:''
    };
    $scope.updatePropertyInModel($scope.property);
    console.log($scope.property)
    // console.log( actObj.setuser.consumerarr)
    $scope.close();
    $scope.$apply();
    setTimeout(function () {
  ...
  $window.closePerson = $scope.close = function () {
    handleAssignmentInput($scope);
    ...
    $http({
      ...
      .success(function (data, status, headers, config) {
        ...
        for (var i = 0; i < obj.length; i++) {
          ...
          roles.push({ id: obj[i].rid, name: obj[i].rolename });
        }
        $scope.roles = roles;
      })
    ...
    $http({
      ...
      .success(function (data, status, headers, config) {
        ...
        if (data != null) {
          var accounts = [];
          for (var i = 0; i < obj.length; i++) {
            accounts.push({ id: obj[i].uid, name: obj[i].username, tel: obj[i].tel, age: obj[i].age, index: i });
          }
  ...
  $scope.getAllRoles = function (successCallback) {
    $http({
      ...
      .success(function (data, status, headers, config) {
        var obj = data;
        for (var i = 0; i < obj.length; i++) {
          candidateGroups.push({ id: obj[i].rid, name: obj[i].rolename });
        }
